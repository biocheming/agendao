use crate::web_tool_runtime::{
    convert_html_to_markdown, ensure_http_url, ensure_safe_http_url, extract_title, strip_html,
    Metadata, PermissionRequest, Tool, ToolContext, ToolError, ToolResult, WebError,
...
fn map_web_error(error: WebError) -> ToolError {
    match error {
        WebError::InvalidArguments(message) => ToolError::InvalidArguments(message),
        WebError::ExecutionError(message) => ToolError::ExecutionError(message),
        WebError::PermissionDenied(message) => ToolError::PermissionDenied(message),
    }
}
...
This is not a JS browser. It is the single authority for stateful web page navigation in AgenDao."#;
    ...
    match input.operation {
        BrowserSessionOperation::Start => {
            if let Some(base_url) = input
                ...
                .filter(|value| !value.is_empty())
            {
                ensure_http_url(base_url).map_err(map_web_error)?;
            }
        }
        BrowserSessionOperation::Visit => {
            ...
            if target.starts_with("http://") || target.starts_with("https://") {
                ensure_http_url(target).map_err(map_web_error)?;
            }
        }
...
async fn resolve_target_url(
    ...
    if target.starts_with("http://") || target.starts_with("https://") {
        return ensure_safe_http_url(target).await.map_err(map_web_error);
    }
    ...
    if let Some(current) = current {
        ...
        return ensure_safe_http_url(joined.as_str())
            .await
            .map_err(map_web_error);
    }
    drop(state);
    ...
    ensure_safe_http_url(joined.as_str())
        .await
        .map_err(map_web_error)
}
...
mod tests {
    ...
    fn browser_session_input_accepts_snake_case_fields() {
        ...
        assert_eq!(input.user_agent.as_deref(), Some("agendao-test"));
    }
        ...
        let handle = tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    let cookie_present = request.contains("agendao_session=alpha");
                    let response = match path {
                        "/set-cookie" => http_response(
                            200,
                            &["Content-Type: text/html; charset=utf-8", "Set-Cookie: agendao_session=alpha; Path=/"],
