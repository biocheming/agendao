use crate::web_tool_runtime::{
    build_web_client, convert_html_to_markdown, ensure_safe_http_url, network_scope_key,
    strip_html, PermissionRequest, Tool, ToolContext, ToolError, ToolResult, WebError,
...
impl Tool for WebFetchTool {
    ...
    async fn execute(
        ...
        let safe_url = ensure_safe_http_url(&url).await.map_err(map_web_error)?;
        let url = safe_url.to_string();

        ctx.ask_permission(
            PermissionRequest::new("webfetch")
                .with_pattern(&url)
                .with_scope_key(network_scope_key(&url).unwrap_or_else(|| format!("net:{url}")))
                .always_allow(),
        )
        .await?;
