...
pub use browser_session::BrowserSessionTool;
pub use codesearch::CodeSearchTool;
pub use github_research::GitHubResearchTool;
pub use webfetch::WebFetchTool;
pub use websearch::WebSearchTool;

pub(crate) mod web_tool_runtime {
    pub(crate) use crate::git_runtime::{ensure_git_available, run_git_command};
    pub use crate::{
        build_web_client, convert_html_to_markdown, ensure_http_url, ensure_safe_http_url,
        extract_title, strip_html, WebError, DEFAULT_WEB_TIMEOUT_SECS, DEFAULT_WEB_USER_AGENT,
        MAX_WEB_RESPONSE_SIZE, MAX_WEB_TIMEOUT_SECS,
    };
    pub use agendao_tool_core::{
        network_scope_key, Metadata, PermissionRequest, Tool, ToolContext, ToolError, ToolResult,
    };
}

pub const MAX_WEB_RESPONSE_SIZE: usize = 5 * 1024 * 1024;
pub const DEFAULT_WEB_TIMEOUT_SECS: u64 = 30;
pub const MAX_WEB_TIMEOUT_SECS: u64 = 120;
pub const DEFAULT_WEB_USER_AGENT: &str =
...
pub fn build_web_client() -> Client {
...
pub fn ensure_http_url(url: &str) -> Result<(), WebError> {
    let parsed = parse_http_url(url)?;
    validate_http_url_host(&parsed)?;
    Ok(())
}

pub async fn ensure_safe_http_url(url: &str) -> Result<Url, WebError> {
    let parsed = parse_http_url(url)?;
    validate_http_url_host(&parsed)?;
...
fn parse_http_url(url: &str) -> Result<Url, WebError> {
    let parsed = Url::parse(url)
        .map_err(|error| WebError::InvalidArguments(format!("invalid URL `{}`: {}", url, error)))?;

    match parsed.scheme() {
        "http" | "https" => Ok(parsed),
        _ => Err(WebError::InvalidArguments(
...
fn validate_http_url_host(parsed: &Url) -> Result<(), WebError> {
    ...
    match host {
        Host::Ipv4(ip) if is_blocked_ip(IpAddr::V4(ip)) => Err(WebError::PermissionDenied(
            format!("refusing to access local or private address `{}`", ip),
        )),
        Host::Ipv6(ip) if is_blocked_ip(IpAddr::V6(ip)) => Err(WebError::PermissionDenied(
            format!("refusing to access local or private address `{}`", ip),
        )),
        Host::Domain(domain) if is_blocked_host(domain) => Err(WebError::PermissionDenied(
...
async fn validate_resolved_host(parsed: &Url) -> Result<(), WebError> {
    ...
    let port = parsed.port_or_known_default().ok_or_else(|| {
        WebError::InvalidArguments(format!("URL `{}` must include a valid port", parsed))
    })?;

    let resolved = tokio::net::lookup_host((domain, port))
        .await
        .map_err(|error| {
            WebError::ExecutionError(format!("failed to resolve host `{}`: {}", domain, error))
        })?;

    for addr in resolved {
        if is_blocked_ip(addr.ip()) {
            return Err(WebError::PermissionDenied(format!(
