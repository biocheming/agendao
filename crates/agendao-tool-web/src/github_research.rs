use crate::web_tool_runtime::{
    run_git_command, Metadata, PermissionRequest, Tool, ToolContext, ToolError, ToolResult,
};
...
impl GitHubResearchTool {
    ...
    async fn run_git(
        ...
        run_git_command(&args, cwd, ctx, DEFAULT_GIT_TIMEOUT_SECS).await
    }
    ...
    async fn fetch_json<T: for<'de> Deserialize<'de>>(
        ...
        let mut request = self
            .client
            .get(url)
            .header(header::USER_AGENT, "agendao-github-research")
...
fn ensure_git_available() -> Result<(), ToolError> {
    crate::web_tool_runtime::ensure_git_available()
}
