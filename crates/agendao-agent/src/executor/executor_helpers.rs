...
use agendao_orchestrator::ToolExecError as OrchestratorToolExecError;
use agendao_provider::ProviderRegistry;
use agendao_tool::{ToolContext, ToolError, ToolRegistry};
use agendao_types::{
...
pub(super) fn append_provider_message(
    ...
    message: &agendao_provider::Message,
    ...
    match message.role {
        ...
        agendao_provider::Role::Assistant => match &message.content {
        ...
        agendao_provider::Role::Tool => {
            if let agendao_provider::Content::Parts(parts) = &message.content {
...
pub(super) fn extract_text_from_provider_content(content: &agendao_provider::Content) -> String {
    match content {
...
pub(super) fn extract_attachments_from_provider_content(
    content: &agendao_provider::Content,
) -> Vec<crate::Attachment> {
    match content {
        agendao_provider::Content::Text(_) => Vec::new(),
        agendao_provider::Content::Parts(parts) => parts
            .iter()
            .filter_map(|part| {
...
pub(super) fn attach_subsession_callbacks(
    ...
    let ctx = ctx.with_get_agent_info({
        ...
        move |name| {
            ...
            async move {
                Ok(registry.get(&name).map(|info| agendao_tool::TaskAgentInfo {
        ...
        move |name, system_prompt, model, max_steps, allowed_tools| {
            ...
            async move {
                ...
                if let Some(info) = registry.get(&name) {
                    return Ok(agendao_tool::TaskAgentInfo {
                        name: info.name.clone(),
                        model: info.model.as_ref().map(|m| agendao_tool::TaskAgentModel {
                            provider_id: m.provider_id.clone(),
                            model_id: m.model_id.clone(),
                ...
                if !allowed_tools.is_empty() {
                    ...
                    let ruleset = agendao_permission::build_agent_ruleset(&name, &[]);
                    agent.allowed_tools = inherited;
                    agent = agent.with_permission(ruleset);
                }

                Ok(agendao_tool::TaskAgentInfo {
                    name: agent.name.clone(),
                    model: agent.model.as_ref().map(|m| agendao_tool::TaskAgentModel {
                        provider_id: m.provider_id.clone(),
                        model_id: m.model_id.clone(),
                    }),
...
mod tests {
    ...
    fn append_provider_message_preserves_assistant_reasoning() {
        let provider_message = agendao_provider::Message::assistant_parts(vec![
            agendao_provider::ContentPart::reasoning("internal trace".to_string()),
            agendao_provider::ContentPart::text("visible answer".to_string()),
            agendao_provider::ContentPart::tool_use(
