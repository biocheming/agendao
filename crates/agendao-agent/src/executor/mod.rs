...
use agendao_orchestrator::runtime::bridges::{ModelCallerBridge, ToolDispatcherBridge};
...
#[cfg(test)]
use agendao_tool::ToolError;
use agendao_tool::ToolRegistry;
...
impl AgentExecutor {
    ...
    where
        F: Fn(Vec<agendao_tool::QuestionDef>) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<Vec<Vec<String>>, agendao_tool::ToolError>>
            + Send
            + 'static,
    {
    ...
    where
        F: Fn(agendao_tool::PermissionRequest) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = Result<(), agendao_tool::ToolError>> + Send + 'static,
    {
    ...
    fn load_runtime_messages(&mut self, messages: &[agendao_provider::Message]) {
    ...
    fn persisted_runtime_messages(
        &self,
        persisted_messages: &[agendao_provider::Message],
        mut runtime_messages: Vec<agendao_provider::Message>,
    ) -> Vec<agendao_provider::Message> {
        ...
        match persisted_messages.first() {
            Some(first) if matches!(first.role, agendao_provider::Role::System) => {
...
fn system_message_text(message: &agendao_provider::Message) -> Option<&str> {
    match (&message.role, &message.content) {
        (agendao_provider::Role::System, agendao_provider::Content::Text(text)) => {
            Some(text.as_str())
        }
        _ => None,
    }
...
mod tests {
    ...
    use agendao_orchestrator::runtime::traits::LoopSink;
    use agendao_permission::{PermissionAction, PermissionRule};
    ...
    fn persisted_runtime_messages_restore_original_system_prompt() {
        ...
        runtime_messages.push(agendao_provider::Message::assistant("done"));
        ...
        assert!(persisted_runtime
            .last()
            .is_some_and(|message| matches!(message.role, agendao_provider::Role::Assistant)));
    }
    ...
    fn persisted_runtime_messages_drop_inserted_request_scope_system_prompt() {
        ...
        persisted_messages.push(agendao_provider::Message::user("hello"));
        let mut runtime_messages =
            executor.apply_request_skill_tree_plan(persisted_messages.clone());
        runtime_messages.push(agendao_provider::Message::assistant("done"));
        ...
        assert!(persisted_runtime
            .first()
            .is_some_and(|message| matches!(message.role, agendao_provider::Role::User)));
        assert_eq!(
    ...
    fn mock_stream(events: Vec<StreamEvent>) -> agendao_provider::StreamResult {
    ...
    async fn process_stream_fixture(
        mut stream: agendao_provider::StreamResult,
    ...
    async fn streaming_sink_maps_runtime_events_to_render_events() {
        ...
        sink.on_event(&LoopEvent::StepDone {
            ...
            usage: Some(agendao_orchestrator::runtime::events::StepUsage {
                prompt_tokens: 10,
