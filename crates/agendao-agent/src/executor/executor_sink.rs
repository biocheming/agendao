...
use agendao_orchestrator::{classify_tool_error_message, ERROR_CAUSE_KEY};
...
impl AgentLoopSink {
    fn flush_assistant_with_tools(&mut self) {
        ...
        if !self.step_reasoning.is_empty() {
            parts.push(agendao_provider::ContentPart {
        ...
        for tool_call in &self.step_tool_calls {
            parts.push(agendao_provider::ContentPart {
                ...
                text: None,
                image_url: None,
                tool_use: Some(agendao_provider::ToolUse {
        ...
        self.messages.push(agendao_provider::Message {
            role: agendao_provider::Role::Assistant,
            content: agendao_provider::Content::Parts(parts),
            cache_control: None,
            provider_options: None,
        });
...
impl LoopSink for AgentLoopSink {
    ...
    async fn on_tool_result(
        ...
        self.messages.push(agendao_provider::Message {
            role: agendao_provider::Role::Tool,
            content: agendao_provider::Content::Parts(vec![agendao_provider::ContentPart {
                content_type: "tool_result".to_string(),
                text: None,
                image_url: None,
                tool_use: None,
                tool_result: Some(agendao_provider::ToolResult {
            ...
            StepBoundary::End { .. } => {
                if !self.assistant_flushed {
                    if !self.step_reasoning.is_empty() {
                        ...
                        let mut parts = vec![agendao_provider::ContentPart {
                            content_type: "reasoning".to_string(),
                            text: Some(self.step_reasoning.clone()),
                            image_url: None,
                            tool_use: None,
                        ...
                        self.messages
                            .push(agendao_provider::Message::assistant(self.step_text.clone()));
                    }
                    self.assistant_flushed = true;
                }
            }
        ...
        match event {
            ...
            LoopEvent::Error(message) => {
                ...
                return Err(RuntimeLoopError::ModelError(
                    agendao_orchestrator::runtime::events::ModelFailure::Message(message.clone()),
                ));
            }
...
mod tests {
    ...
    use agendao_orchestrator::runtime::events::LoopError as RuntimeLoopError;
