...
pub(super) fn get_managed_config_dir() -> PathBuf {
    if let Ok(test_dir) = env::var("AGENDAO_TEST_MANAGED_CONFIG_DIR") {
        return PathBuf::from(test_dir);
    }
    if cfg!(target_os = "macos") {
        PathBuf::from("/Library/Application Support/agendao")
    } else if cfg!(target_os = "windows") {
        let program_data =
            env::var("ProgramData").unwrap_or_else(|_| "C:\\ProgramData".to_string());
        PathBuf::from(program_data).join("agendao")
    } else {
        PathBuf::from("/etc/agendao")
    }
}

/// Collect .agendao directories from project hierarchy and global config.
pub(super) fn collect_agendao_directories(project_dir: &Path) -> Vec<PathBuf> {
    ...
    // Only the current workspace-local `.agendao` directory participates in
    // config/command/agent discovery. Ancestor `.agendao` directories do not.
    let start_dir = normalize_existing_path(project_dir);
    let local_agendao_dir = start_dir.join(".agendao");
    if local_agendao_dir.is_dir() {
    ...
    if let Ok(config_dir) = env::var("AGENDAO_CONFIG_DIR") {
