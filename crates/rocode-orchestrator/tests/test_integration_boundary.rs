...
use rocode_orchestrator::{OrchestrationCore, PromptExecutionOptions, SessionStore};
...
impl rocode_provider::Provider for MockBoundaryProvider {
    ...
    async fn chat_stream(
        ...
        Err(rocode_provider::ProviderError::ApiError("streaming not implemented".into()))
    }
}
...
async fn test_user_message_carries_canonical_source_metadata() {
    ...
    let opts = PromptExecutionOptions {
        ...
        source_surface: Some(rocode_types::MessageSourceSurface::Tui),
        ..Default::default()
    };
    ...
    let origin = rocode_types::message_source_origin(&first_msg.metadata);
...
// DirectTransport<rocode_session::SessionManager> wiring is tested at the
// rocode-client crate level (test_transport_selector_fallback.rs).
