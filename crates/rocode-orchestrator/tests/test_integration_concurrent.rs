...
impl rocode_provider::Provider for MockConcurrentProvider {
    fn id(&self) -> &str {
        "mock-concurrent"
    }
    ...
    async fn chat(
        &self,
        request: rocode_provider::ChatRequest,
    ) -> Result<rocode_provider::ChatResponse, rocode_provider::ProviderError> {
        // Extract user message
        let user_msg = request
            .messages
            .iter()
            .filter(|m| matches!(m.role, rocode_provider::Role::User))
            .last()
            .and_then(|m| match &m.content {
                rocode_provider::Content::Text(text) => Some(text.as_str()),
                _ => None,
            })
            .unwrap_or("");
