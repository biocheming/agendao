/// Orchestration Core - The single execution authority (ROCode Constitution Article 1)
///
...
///
/// Generic over `S: SessionStore` — default is `rocode_session_core::SessionManager`;
/// `rocode_session::SessionManager` can be injected for unified session authority.

use crate::prompt_execution::{execute_prompt_with_session, execute_prompt_streaming_with_session};
use crate::OrchestratorError;
use rocode_session_core::{SessionAccess, SessionManager, SessionStore};
use std::sync::Arc;
...
impl OrchestrationCore<SessionManager> {
    ...
    pub async fn new(config: &rocode_config::Config) -> Result<Self, OrchestratorError> {
...
impl<S: SessionStore> OrchestrationCore<S> {
    ...
    pub async fn new_with_sessions(
        ...
        let tools = Arc::new(tokio::sync::RwLock::new(rocode_tool::ToolRegistry::new()));
    ...
    pub fn providers(&self) -> &Arc<tokio::sync::RwLock<rocode_provider::ProviderRegistry>> {
        &self.providers
    }

    /// Get tool registry for external registration
    pub fn tools(&self) -> &Arc<tokio::sync::RwLock<rocode_tool::ToolRegistry>> {
        &self.tools
    }
            ...
            messages: record
                ...
                .map(|msg| {
                    let content = msg
                        ...
                        .iter()
                        .filter_map(|part| {
                            if let rocode_types::PartType::Text { text, .. } = &part.part_type {
                                Some(text.as_str())
                            } else {
                                None
                            }
