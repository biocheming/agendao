...
use std::sync::Arc;
use rocode_provider::{ChatRequest, Content, ContentPart, Message, ProviderRegistry, ToolDefinition};
use rocode_session_core::{SessionAccess, SessionStore, MessageRole, PartType};
use rocode_tool::{ToolRegistry, ToolContext};
...
fn extract_response_text(response: &rocode_provider::ChatResponse) -> String {
...
pub async fn execute_prompt_with_session<S: SessionStore>(
    ...
    providers: &Arc<tokio::sync::RwLock<ProviderRegistry>>,
    tools: &Arc<tokio::sync::RwLock<rocode_tool::ToolRegistry>>,
    session_id: &str,
    text: &str,
    options: &PromptExecutionOptions,
) -> Result<PromptExecutionResult, OrchestratorError> {
...
fn extract_response_content(response: &rocode_provider::ChatResponse) -> (String, Vec<ToolCall>) {
    if let Some(choice) = response.choices.first() {
        match &choice.message.content {
...
///
/// Converts `rocode_types` session parts into `rocode_provider` messages:
...
fn extract_text_content(parts: &[rocode_types::MessagePart]) -> String {
    parts
        .iter()
        .filter_map(|part| {
            if let PartType::Text { text, .. } = &part.part_type {
...
pub async fn execute_prompt_streaming_with_session<S: SessionStore + Send + 'static>(
    ...
    struct StreamState<Store: SessionStore> {
        ...
        model_id: String,
        tool_definitions: Vec<rocode_provider::ToolDefinition>,
        options: PromptExecutionOptions,
        current_stream: Option<rocode_provider::StreamResult>,
                                ...
                                if state.accumulated_tool_calls.is_empty() {
                                    ...
                                    match state.provider.chat_stream(request).await {
                                        ...
                                        Err(e) => {
                                            state.finished = true;
                                            return Some((
                                                Err(rocode_provider::ProviderError::StreamError(
