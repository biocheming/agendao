...
impl ScriptedModelResolver {
    fn new(streams: Vec<agendao_provider::StreamResult>) -> Self {
        Self {
    ...
    fn extract_last_user_text(messages: &[agendao_provider::Message]) -> String {
        messages
            .iter()
            .rev()
            .find_map(|message| match (&message.role, &message.content) {
                (agendao_provider::Role::User, agendao_provider::Content::Text(text)) => {
                    Some(text.clone())
                }
                _ => None,
            })
...
fn stream_from_text(text: &str) -> agendao_provider::StreamResult {
    Box::pin(stream::iter(vec![
        Ok::<_, agendao_provider::ProviderError>(agendao_provider::StreamEvent::TextDelta(
            text.to_string(),
        )),
        Ok::<_, agendao_provider::ProviderError>(agendao_provider::StreamEvent::Done),
    ]))
}

fn stream_from_logprob_scores() -> agendao_provider::StreamResult {
    let metadata = json!({
        "logprobs": [[
    ...
    Box::pin(stream::iter(vec![
        ...
        Ok::<_, agendao_provider::ProviderError>(agendao_provider::StreamEvent::FinishStep {
...
fn new_temp_workdir() -> PathBuf {
    let path = std::env::temp_dir().join(format!("agendao-workflow-runtime-{}", now_nanos()));
...
async fn verify_mode_reuses_persistent_pairwise_cache_across_runs() {
    ...
    let run_once = |streams: Vec<agendao_provider::StreamResult>| {
        let executor = Arc::new(ScriptedToolExecutor {
            responses: Mutex::new(VecDeque::from(vec![
                Ok(ToolOutput {
                    output: "score=10".to_string(),
