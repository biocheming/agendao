...
// ---------------------------------------------------------------------------
// Internal conversation state – uses only agendao_provider types.
// ---------------------------------------------------------------------------
...
fn can_retry_initial_stream_fault(
    ...
    match error {
        agendao_provider::ProviderError::Timeout
        | agendao_provider::ProviderError::NetworkError(_) => true,
        agendao_provider::ProviderError::StreamError(message) => {
...
fn stream_termination_from_provider_error(
    error: &agendao_provider::ProviderError,
) -> agendao_provider::StreamTermination {
    match error {
        agendao_provider::ProviderError::NetworkError(_)
        | agendao_provider::ProviderError::Timeout => {
            agendao_provider::StreamTermination::TransportClosed
        }
        agendao_provider::ProviderError::StreamError(message) => {
            agendao_provider::StreamTermination::StreamCorrupt {
                message: message.clone(),
            }
        }
        other => agendao_provider::StreamTermination::ProviderError {
...
struct LoopConversation<'a> {
    messages: &'a mut Vec<agendao_provider::Message>,
}
...
impl<'a> LoopConversation<'a> {
    fn from_messages(messages: &'a mut Vec<agendao_provider::Message>) -> Self {
        Self { messages }
    }

    fn messages(&self) -> &[agendao_provider::Message] {
    ...
    fn replace_messages(&mut self, messages: Vec<agendao_provider::Message>) {
    ...
    fn compact_for_checkpoint(
        ...
        rewritten.push(agendao_provider::Message::assistant(summary.clone()));
...
fn provider_message_summary_fragments(message: &agendao_provider::Message) -> Vec<String> {
...
fn provider_message_context_char_len(message: &agendao_provider::Message) -> usize {
    match &message.content {
        agendao_provider::Content::Text(text) => text.chars().count(),
        agendao_provider::Content::Parts(parts) => parts
            .iter()
            .map(|part| {
                part.text
...
fn is_checkpoint_summary_message(message: &agendao_provider::Message) -> bool {
    matches!(
        ...
        (agendao_provider::Role::Assistant, agendao_provider::Content::Text(text))
...
fn request_view_metrics(messages: &[agendao_provider::Message]) -> RequestViewMetrics {
    ...
    for message in messages {
        match message.role {
            agendao_provider::Role::System => {}
            agendao_provider::Role::User => user_messages += 1,
            agendao_provider::Role::Assistant => {
                assistant_messages += 1;
                if is_checkpoint_summary_message(message) {
                    checkpoint_summary_messages += 1;
                }
            }
            agendao_provider::Role::Tool => tool_messages += 1,
        }
    }
...
pub async fn run_loop<S: LoopSink>(
    ...
    messages: &mut Vec<agendao_provider::Message>,
            ...
            loop {
                    ...
                    .await
                    {
                        ...
                        Err(_elapsed) => {
                            ...
                            return Err(LoopError::ModelErrorWithTermination {
                                ...
                                stream_termination:
                                    agendao_provider::StreamTermination::TransportTimeout,
                            });
                        }
                    }
                } else {
                ...
                if cancel.is_cancelled() {
                    ...
                    return Ok(LoopOutcome {
                        ...
                        total_tool_calls,
                        finish_reason: FinishReason::Cancelled,
                        stream_termination: Some(agendao_provider::StreamTermination::Interrupted),
                    });
                }
