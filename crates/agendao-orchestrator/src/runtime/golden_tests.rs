...
mod tests {
    ...
    fn user_msg(text: &str) -> agendao_provider::Message {
        agendao_provider::Message::user(text.to_string())
    }

    fn repeated_user_messages(count: usize, text: &str) -> Vec<agendao_provider::Message> {
    ...
    async fn golden_provider_stream_error_aborts() {
        ...
        impl ModelCaller for ErrorStreamModel {
            async fn call_stream(&self, _req: LoopRequest) -> Result<StreamResult, LoopError> {
                let items: Vec<Result<StreamEvent, agendao_provider::ProviderError>> = vec![
                    Ok(StreamEvent::TextDelta("partial ".into())),
                    Err(agendao_provider::ProviderError::NetworkError(
        ...
        assert!(matches!(
            ...
            Err(LoopError::ModelError(_))
                | Err(LoopError::ModelErrorWithTermination {
                    stream_termination: agendao_provider::StreamTermination::StreamCorrupt { .. },
                    ..
                })
        ));
