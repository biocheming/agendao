...
//!
//! This module owns the normalized runtime request contract used by sessions,
//! schedulers, agents, compaction, and title generation.
...
use agendao_execution_types::{
    agent_generation_request, compaction_request, message_title_request,
    session_runtime_request_defaults, session_title_request, ExecutionRequestContext,
    ExecutionRequestDefaults,
};
...
impl ResolvedExecutionSpec {
    pub fn compile(&self) -> CompiledExecutionRequest {
        ...
        let mut model_info = self.to_models_dev_info();
        model_info.reasoning = self.model.capabilities.reasoning && !thinking_disabled;
    ...
    fn variant_options(&self, name: &str) -> Option<HashMap<String, serde_json::Value>> {
        ...
        agendao_provider::transform::variants(&self.to_models_dev_info()).remove(name)
    }
...
mod tests {
    ...
    fn cross_entry_point_variant_propagation() {
        ...
        // Both entry points carry the variant through the shared pipeline.
    ...
    fn defaults_field_set_covers_compiled_tunables() {
        ...
        assert!(
            compiled.timeout_secs.is_some(),
            "timeout_secs not propagated"
        );
        assert!(
    ...
    fn compile_leaves_reasoning_effort_none_without_config_or_known_variant() {
        let compiled =
            spec_with_variant(model_spec(None, None), Some("deep"), HashMap::new()).compile();
