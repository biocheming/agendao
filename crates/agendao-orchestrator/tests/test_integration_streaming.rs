...
impl agendao_provider::Provider for MockStreamingProvider {
    fn id(&self) -> &str {
        "mock-streaming"
    }
...
async fn test_streaming_e2e() {
    ...
    assert!(events
        .iter()
        .any(|e| matches!(e, Ok(agendao_provider::StreamEvent::Start))));

    // Check for TextDelta events
    let text_deltas: Vec<_> = events
        .iter()
        .filter_map(|e| match e {
            Ok(agendao_provider::StreamEvent::TextDelta(text)) => Some(text.as_str()),
            _ => None,
        })
        .collect();
    ...
    assert!(events
        .iter()
        .any(|e| matches!(e, Ok(agendao_provider::StreamEvent::Done))));
...
async fn test_streaming_event_order() {
    ...
    for event in events {
        match event {
            Ok(agendao_provider::StreamEvent::Start) => {
                assert!(!saw_start);
                saw_start = true;
            }
            Ok(agendao_provider::StreamEvent::TextDelta(_)) => {
                assert!(saw_start);
                assert!(!saw_finish);
                assert!(!saw_done);
                saw_text = true;
            ...
            Ok(agendao_provider::StreamEvent::Done) => {
                assert!(saw_start);
                assert!(saw_finish);
                assert!(!saw_done);
                saw_done = true;
