...
// Default URL for the Anthropic-compatible messages wire shape. Users override
// this via `base_url` in their provider configuration.
const ANTHROPIC_DEFAULT_URL: &str = "https://api.anthropic.com/v1/messages";

/// Build the Anthropic-compatible endpoint URL from a user-supplied base URL.
/// The generic anthropic-family transport appends `/messages` when the base URL
...
fn anthropic_url(base_url: &str) -> String {
    ...
    if base.is_empty() {
        return ANTHROPIC_DEFAULT_URL.to_string();
    }
        ...
        for msg in sanitized {
            match msg.role {
                ...
                crate::Role::Assistant => {
                    let Some(message) = assistant_message_to_anthropic(msg) else {
                        continue;
                    };
                    messages.push(message);
                }
                crate::Role::User | crate::Role::Tool => {
                    let content = prompt_message_to_anthropic_user_blocks(&msg);
                    if content.is_empty() {
                        continue;
                    }
...
fn assistant_message_to_anthropic(message: Message) -> Option<MessagesMessage> {
...
fn prompt_message_to_anthropic_user_blocks(message: &Message) -> Vec<MessagesContent> {
    if matches!(message.role, Role::User) {
        return user_textual_content_to_anthropic(&message.content);
    }
...
fn user_textual_content_to_anthropic(content: &crate::Content) -> Vec<MessagesContent> {
    match content {
        crate::Content::Text(text) => {
            if text.is_empty() {
                Vec::new()
...
impl ProviderAdapter for AnthropicAdapter {
    async fn chat(
        ...
        tracing::debug!(url = %url, model = %request.model, "anthropic adapter chat request");
        ...
        if !response.status().is_success() {
            ...
            tracing::error!(url = %url, status = %status, "anthropic adapter chat error");
    ...
    async fn chat_stream(
        ...
        tracing::debug!(
            url = %url,
            model = %request.model,
            "anthropic adapter chat_stream request"
        );
        ...
        if !response.status().is_success() {
            ...
            tracing::error!(
                url = %url,
                status = %status,
                "anthropic adapter chat_stream error"
            );
            return Err(ProviderError::ApiError(format!("{}: {}", status, body)));
        }
                ...
                match json_stream.next().await {
                    Some(Ok(value)) => {
                        ...
                        if let Some(ref e) = event {
                            tracing::trace!(event = ?e, "anthropic adapter sse event");
                        }
...
mod tests {
    ...
    #[test]
    fn anthropic_url_empty_falls_back_to_default() {
        assert_eq!(anthropic_url(""), ANTHROPIC_DEFAULT_URL);
        assert_eq!(anthropic_url("  "), ANTHROPIC_DEFAULT_URL);
    }
    ...
    fn anthropic_url_appends_messages_path() {
        assert_eq!(
            anthropic_url("https://coding.dashscope.aliyuncs.com/apps/anthropic/v1"),
            "https://coding.dashscope.aliyuncs.com/apps/anthropic/v1/messages"
        );
    }

    #[test]
    fn anthropic_url_no_double_append() {
        assert_eq!(
            anthropic_url("https://example.com/v1/messages"),
            "https://example.com/v1/messages"
        );
    }

    #[test]
    fn anthropic_url_strips_trailing_slash() {
        assert_eq!(
            anthropic_url("https://example.com/v1/"),
            "https://example.com/v1/messages"
        );
    }

    #[test]
    fn anthropic_url_appends_v1_like_connection_test() {
        ...
        assert_eq!(
            anthropic_url("https://astron.example.com/v2"),
    ...
    #[test]
    // P2.3: reused call_id history must be sanitized before Anthropic (messages) transport.
    ...
    #[test]
    fn p2_downgraded_tool_summary_stays_role_user_on_anthropic_wire() {
        let request = ChatRequest {
            model: "claude-test".to_string(),
            messages: vec![Message {
                role: Role::User,
    ...
    // P2 canonical ordering: the Anthropic adapter preserves input order and
    ...
    #[test]
    fn p2_canonical_input_order_preserved_on_anthropic_wire() {
        let request = ChatRequest {
            model: "claude-test".to_string(),
            messages: vec![Message {
                role: Role::Assistant,
