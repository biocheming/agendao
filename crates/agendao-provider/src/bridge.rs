...
/// Convert a single `StreamingEvent` into zero or more agendao `StreamEvent`s.
///
...
pub fn streaming_event_to_stream_events(event: StreamingEvent) -> Vec<StreamEvent> {
    match event {
        ...
        StreamingEvent::ToolCallEnded { .. } => {
            // ToolCallEnd in agendao requires assembled input (serde_json::Value).
        ...
        StreamingEvent::FinalCandidate { .. } => {
            // Multi-candidate scenarios are not used in agendao.
            vec![]
        }
...
/// Convert a stream of `StreamingEvent`s into a agendao `StreamResult`.
///
...
/// Convert a `DriverResponse` into a agendao `ChatResponse`.
///
...
///
/// ```ignore
/// use agendao_provider::driver::{ApiStyle, DriverMessage};
///
...
impl ProviderAdapter for DriverBasedAdapter {
    async fn chat(
        ...
        // Convert agendao ChatRequest → driver params
...
/// Convert agendao `ChatRequest` messages into `DriverMessage` format.
///
