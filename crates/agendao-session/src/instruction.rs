...
fn is_project_config_disabled() -> bool {
    env_truthy_any(&["AGENDAO_DISABLE_PROJECT_CONFIG"])
}
fn agendao_config_dir_env() -> Option<String> {
    env_var_any(&["AGENDAO_CONFIG_DIR"])
}
...
fn global_files() -> Vec<PathBuf> {
    ...
    // AGENDAO_CONFIG_DIR override comes first.
    if let Some(dir) = agendao_config_dir_env() {
...
impl InstructionLoader {
    ...
    fn resolve_config_path(
        ...
        if path.is_absolute() {
            ...
            } else if let Some(dir) = agendao_config_dir_env() {
                glob_up(&expanded, Path::new(&dir), Path::new(&dir))
            } else {
                warn!(
                    "Skipping relative instruction \"{}\" - no AGENDAO_CONFIG_DIR set while project config is disabled",
                    instruction
                );
                Vec::new()
            }
