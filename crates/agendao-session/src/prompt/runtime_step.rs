...
pub(super) struct SessionToolExecutor {
    ...
    pub(super) tool_ctx_builder: Arc<dyn Fn() -> agendao_tool::ToolContext + Send + Sync>,
    pub(super) allowed_tools: Option<Arc<std::collections::HashSet<String>>>,
}

#[async_trait]
impl agendao_orchestrator::ToolExecutor for SessionToolExecutor {
    async fn execute(
        &self,
        tool_name: &str,
        arguments: serde_json::Value,
        _exec_ctx: &agendao_orchestrator::ExecutionContext,
        ...
        let result = self
            ...
            .await
            .map_err(|e| match e {
                agendao_tool::ToolError::InvalidArguments(msg) => {
                    agendao_orchestrator::ToolExecError::InvalidArguments(msg)
                }
                agendao_tool::ToolError::PermissionDenied(msg) => {
                    agendao_orchestrator::ToolExecError::PermissionDenied(msg)
                }
                agendao_tool::ToolError::Cancelled => {
                    agendao_orchestrator::ToolExecError::ExecutionError("cancelled".to_string())
                }
                other => agendao_orchestrator::ToolExecError::ExecutionError(other.to_string()),
            })?;
        Ok(agendao_orchestrator::ToolOutput {
...
impl ToolDispatcher for SessionStepToolDispatcher {
    async fn execute(&self, call: &RuntimeToolCallReady) -> RuntimeToolResult {
        ...
        let tool_runner = agendao_orchestrator::ToolRunner::new(executor);
        let exec_ctx = agendao_orchestrator::ExecutionContext {
        ...
        let input = agendao_orchestrator::tool_runner::ToolCallInput {
            id: call.id.clone(),
        ...
        match event {
            ...
            LoopEvent::StepDone {
                ...
                self.finish_reason = Some(match finish_reason {
                    agendao_orchestrator::runtime::events::FinishReason::ToolUse => {
                        "tool-calls".to_string()
                    }
                    agendao_orchestrator::runtime::events::FinishReason::EndTurn => {
                        "stop".to_string()
                    }
                    agendao_orchestrator::runtime::events::FinishReason::Provider(reason) => {
                        reason.clone()
                    }
                    agendao_orchestrator::runtime::events::FinishReason::MaxSteps => {
                        "max_steps".to_string()
                    }
                    agendao_orchestrator::runtime::events::FinishReason::Cancelled => {
                        "cancelled".to_string()
                    }
                    agendao_orchestrator::runtime::events::FinishReason::Error(message) => {
            ...
            LoopEvent::Error(msg) => {
                ...
                return Err(RuntimeLoopError::ModelError(
                    agendao_orchestrator::runtime::events::ModelFailure::Message(msg.clone()),
                ));
            }
        }
        Ok(())
