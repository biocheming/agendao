...
use agendao_skill::{RuntimeInstructionSource, SkillGovernanceAuthority};
...
fn write_methodology_skill(
    root: &std::path::Path,
    name: &str,
    template: agendao_skill::SkillMethodologyTemplate,
) {
    let skill_dir = root.join(".agendao/skills").join(name);
    fs::create_dir_all(&skill_dir).unwrap();
    let body = agendao_skill::render_methodology_skill_body(name, &template).unwrap();
    fs::write(
...
fn pre_dispatch_governance_forces_compaction_for_small_overflowing_request_view() {
    ...
    let persisted: agendao_types::ContextPressureGovernanceSummary =
    ...
    let lifecycle: agendao_types::ContextCompactionLifecycleSummary =
...
fn pre_dispatch_governance_blocks_when_single_message_remains_over_limit() {
    ...
    assert_eq!(
        lifecycle.status,
        agendao_types::ContextCompactionLifecycleStatus::Failed
    );
}
...
async fn prompt_length_finish_reason_appends_hidden_continuation_turn() {
    ...
    assert!(matches!(last_message.role, agendao_provider::Role::User));
...
fn prepare_skill_reflection_loads_methodology_from_runtime_skill_names() {
    ...
    write_methodology_skill(
        dir.path(),
        "runtime-check",
        agendao_skill::SkillMethodologyTemplate {
...
1. For the harness protocol itself
   ...
   - target path: `.agendao/skills/runtime-check/SKILL.md`
    ...
    governance
        .create_skill(
            agendao_skill::CreateSkillRequest {
                ...
                body:
                    agendao_skill::render_methodology_skill_body(
                        "runtime-check",
                        &agendao_skill::SkillMethodologyTemplate {
                            when_to_use: vec![
