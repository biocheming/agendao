...
pub enum ExternalAdapterIngressMappingError {
    #[error("external adapter event has no resolved AgenDao session binding")]
...
mod tests {
    ...
    fn sample_external_event() -> ExternalAdapterEvent {
        ExternalAdapterEvent {
            ...
            attachments: vec![agendao_types::ExternalAdapterAttachmentRef {
                id: "file_1".to_string(),
                kind: "image".to_string(),
                uri: "agendao://external/generic/file_1".to_string(),
            }],
            idempotency_key: None,
            reply_target: Some(agendao_types::ExternalAdapterReplyTarget {
                target_type: "chat".to_string(),
                target_id: "chat_1".to_string(),
                thread_id: Some("thread_1".to_string()),
            }),
            raw_event_ref: Some(agendao_types::ExternalAdapterRawEventRef {
                kind: "object-ref".to_string(),
                uri: "agendao://external/generic/evt_1".to_string(),
                checksum: None,
            }),
        }
    }
    ...
    fn external_adapter_metadata_does_not_enter_shadow_prompt_text() {
        ...
        assert_eq!(
            ...
            "agendao://external/generic/evt_1"
        );
    }
    ...
    fn external_adapter_attachments_remain_refs() {
        ...
        assert_eq!(turn.attachments[0].uri, "agendao://external/generic/file_1");
