...
mod tests {
    use crate::{
        MemoryArtifactBundle, MemoryArtifactVersion, MemoryEvidenceRef, MemoryKind, MemoryRecord,
        MemoryRecordId, MemoryScope, MemoryStatus, MemoryValidationStatus,
    };
    ...
    fn import_envelope_rejects_unknown_bundle_version() {
        let payload = serde_json::json!({
            "version": "agendao-rust/memory/v999",
            "exported_at": 123,
            "records": [sample_record()]
        });

        let error = serde_json::from_value::<MemoryArtifactBundle>(payload)
    ...
    fn import_envelope_rejects_malformed_record_payload() {
        let payload = serde_json::json!({
            "version": "agendao-rust/memory/v1",
            "exported_at": 123,
            "records": [{
                "id": 42,
                "title": "broken"
