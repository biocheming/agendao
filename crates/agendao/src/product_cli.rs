...
const MANAGED_BINARIES: [&str; 1] = ["agendao"];
...
enum ProductCommand {
    ...
    Tui(TuiArgs),
    #[command(about = "Attach TUI to a running AgenDao server")]
    Attach(AttachArgs),
    ...
    Generate,
    #[command(about = "Upgrade agendao to latest or specific version")]
    Upgrade {
    ...
    #[command(about = "Uninstall agendao and remove related files")]
    Uninstall {
...
pub async fn dispatch_if_product_command(args: Vec<OsString>) -> anyhow::Result<bool> {
    ...
    match cli.command {
        ...
        Some(ProductCommand::Generate) => {
            agendao_server::print_openapi_spec().await?;
        }
...
fn managed_web_asset_dir() -> Option<PathBuf> {
    if let Some(bin_dir) = managed_binary_dir() {
        if let Some(prefix) = bin_dir.parent() {
            return Some(prefix.join("share").join("agendao").join("web"));
        }
    }
    None
}
...
fn detect_install_method() -> InstallMethod {
    ...
    let checks: &[(InstallMethod, &str, &[&str], &str)] = &[
        ...
        (InstallMethod::Bun, "bun", &["pm", "ls", "-g"], "agendao-ai"),
        (
            InstallMethod::Brew,
            "brew",
            &["list", "--formula", "agendao"],
            "agendao",
        ),
        (
            InstallMethod::Choco,
    ...
    match method {
        ...
        | InstallMethod::Unknown => {
            let json: serde_json::Value = client
                .get("https://api.github.com/repos/anomalyco/agendao/releases/latest")
                .header("User-Agent", "agendao")
                .send()
                .await?
                .json()
                .await?;
        ...
        InstallMethod::Brew => {
            let json: serde_json::Value = client
                .get("https://formulae.brew.sh/api/formula/agendao.json")
                .send()
                .await?
                .json()
                .await?;
        ...
        InstallMethod::Npm | InstallMethod::Pnpm | InstallMethod::Bun => {
            let json: serde_json::Value = client
                .get("https://registry.npmjs.org/agendao-ai/latest")
                .send()
                .await?
                .json()
                .await?;
        ...
        InstallMethod::Scoop => {
            let json: serde_json::Value = client
                .get("https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/agendao.json")
                .send()
                .await?
                .json()
                .await?;
...
fn run_upgrade_process(method: InstallMethod, target: &str) -> anyhow::Result<()> {
    let status = match method {
        InstallMethod::Local => {
            anyhow::bail!(
                "Local installs package AgenDao as a single `agendao` binary. Reinstall from the matching release bundle or rerun `./scripts/install-local.sh release` from a source checkout."
            );
        }
        InstallMethod::Curl => ProcessCommand::new("sh")
            .arg("-c")
            .arg("curl -fsSL https://agendao.dev/install | bash")
            .env("VERSION", target)
            .status(),
        InstallMethod::Npm => ProcessCommand::new("npm")
            .args(["install", "-g", &format!("agendao-ai@{}", target)])
            .status(),
        ...
        InstallMethod::Brew => ProcessCommand::new("brew")
            .args(["upgrade", "agendao"])
            .status(),
        InstallMethod::Choco => ProcessCommand::new("choco")
            .args(["upgrade", "agendao", "--version", target, "-y"])
            .status(),
        InstallMethod::Scoop => ProcessCommand::new("scoop")
            .args(["install", &format!("agendao@{}", target)])
            .status(),
        InstallMethod::Unknown => {
...
async fn handle_upgrade_command(
    ...
    if current == target {
        println!("agendao upgrade skipped: {} is already installed", target);
        return Ok(());
    }
...
fn print_build_info() {
    let built_at = option_env!("AGENDAO_BUILD_TIMESTAMP").unwrap_or("unknown");
    let rustc = option_env!("AGENDAO_RUSTC_VERSION").unwrap_or("unknown");
    let profile = option_env!("AGENDAO_BUILD_PROFILE").unwrap_or("unknown");
    let target = option_env!("TARGET").unwrap_or("unknown");
    let host = option_env!("HOST").unwrap_or("unknown");

    println!("AgenDao {}", env!("CARGO_PKG_VERSION"));
    println!();
    println!("Build Info:");
