...
1. For `harness/skills/evaluate_properties.md`
   ...
   - target path: `.agendao/skills/drug-discovery-evaluate-properties/SKILL.md`
...
4. For the harness protocol itself
   ...
   - target path: `.agendao/skills/drug-discovery-harness/SKILL.md`
...
fn hub_store_persists_operational_snapshots() {
    ...
    store
        .upsert_skill_operational_snapshot(SkillOperationalSnapshot {
            ...
            source_id: None,
            usage: Some(agendao_types::SkillUsageLedgerEntry {
            ...
            writes: Some(agendao_types::SkillWriteLedgerEntry {
                ...
                remove_count: 0,
                delete_count: 0,
                last_action: Some(agendao_types::SkillWriteLedgerAction::Create),
    ...
    assert_eq!(
        ...
        Some(agendao_types::SkillWriteLedgerAction::Create)
    );
}
...
fn governance_negative_entropy_diagnostics_flag_never_reused_workspace_skill() {
    ...
    assert_eq!(
        candidate.severity,
        agendao_types::SkillGovernanceDiagnosticSeverity::Warn
    );
}
...
fn governance_negative_entropy_downgrades_recent_positive_evolution_signal() {
    ...
    assert_eq!(
        candidate.severity,
        agendao_types::SkillGovernanceDiagnosticSeverity::Info
    );
    assert!(candidate
        .reasons
        .iter()
...
fn governance_sync_negative_entropy_review_candidates_marks_workspace_skill() {
    ...
    assert_eq!(
        ...
        Some(agendao_types::SkillVitalityState::ReviewCandidate)
    );
    assert_eq!(
        updated[0]
            .vitality
        ...
        Some(agendao_types::SkillRetirementReasonKind::NegativeEntropy)
    );
}
...
fn governance_runtime_availability_rejects_retired_skill() {
    ...
    assert!(matches!(
        ...
        SkillError::SkillRuntimeUnavailable {
            name,
            state: agendao_types::SkillVitalityState::Retired
...
fn governance_semantic_conflict_diagnostics_use_usage_ledger_to_prioritize_pair() {
    let dir = tempdir().unwrap();
    let root = dir.path().join(".agendao/skills");
...
"#,
    ...
    assert!(matches!(
        pair.kind,
        agendao_types::SkillSemanticConflictKind::ReplacementHint
            | agendao_types::SkillSemanticConflictKind::NearDuplicate
    ));
...
"#,
    ...
    assert_eq!(
        updated[0]
            ...
            .as_ref()
            .map(|record| record.reason.kind),
        Some(agendao_types::SkillRetirementReasonKind::SemanticConflict)
    );
    assert_eq!(
        updated[0]
            .vitality
...
fn governance_runtime_skill_composition_hints_surface_canonical_and_complementary_guidance() {
    ...
    assert!(hints.iter().any(|hint| {
        hint.kind == agendao_types::SkillRuntimeCompositionHintKind::PreferCanonicalSkill
    ...
    assert!(hints.iter().any(|hint| {
        hint.kind == agendao_types::SkillRuntimeCompositionHintKind::ComplementaryBundle
...
"#,
    ...
    assert_eq!(guard_report.status, agendao_types::SkillGuardStatus::Warn);
    assert!(guard_report
        .violations
        .iter()
...
fn governance_run_guard_for_source_warns_on_semantic_overlap() {
    let dir = tempdir().unwrap();
    let workspace_root = dir.path().join(".agendao/skills");
...
fn governance_timeline_merges_managed_provenance_and_audit_entries() {
    ...
    let entries = governance.governance_timeline(&agendao_types::SkillHubTimelineQuery {
        skill_name: Some("timeline-skill".to_string()),
        source_id: None,
        limit: None,
    });
...
fn governance_create_returns_guard_warning_and_audits_it() {
    ...
    assert_eq!(report.status, agendao_types::SkillGuardStatus::Warn);
...
fn guard_run_for_skill_returns_report_without_write_path() {
    ...
    assert_eq!(reports[0].status, agendao_types::SkillGuardStatus::Warn);
