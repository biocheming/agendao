...
pub(crate) const SKILL_GOVERNANCE_DIR: &str = ".agendao/state/skill";
...
const ARTIFACT_CACHE_SCHEMA: &str = "agendao.skill_artifact_cache";
const ARTIFACT_CACHE_VERSION: u32 = 1;
const COMPOSITION_GRAPH_SCHEMA: &str = "agendao.skill_composition_graph";
const COMPOSITION_GRAPH_VERSION: u32 = 1;
const DISTRIBUTION_LOCK_SCHEMA: &str = "agendao.skill_distribution_lock";
const DISTRIBUTION_LOCK_VERSION: u32 = 1;
const LIFECYCLE_SCHEMA: &str = "agendao.skill_lifecycle";
const LIFECYCLE_VERSION: u32 = 1;
const OPERATIONAL_LEDGER_SCHEMA: &str = "agendao.skill_operational_ledger";
const OPERATIONAL_LEDGER_VERSION: u32 = 1;
const SOURCE_INDEX_CACHE_SCHEMA: &str = "agendao.skill_source_index_cache";
...
fn merge_operational_snapshot(
    ...
    if matches!(
        ...
        agendao_types::SkillOperationalSourceScope::Unknown
    ) {
...
fn merge_skill_usage_entry(
    existing: &mut SkillOperationalSnapshot,
    incoming: Option<agendao_types::SkillUsageLedgerEntry>,
) {
...
fn merge_skill_write_entry(
    existing: &mut SkillOperationalSnapshot,
    incoming: Option<agendao_types::SkillWriteLedgerEntry>,
) {
    let Some(incoming) = incoming else {
        return;
    };
...
fn merge_skill_vitality_entry(
    existing: &mut SkillOperationalSnapshot,
    incoming: Option<agendao_types::SkillVitalityRecord>,
) {
    let Some(incoming) = incoming else {
        return;
    };
...
fn merge_skill_evolution_entry(
    existing: &mut SkillOperationalSnapshot,
    incoming: Option<agendao_types::SkillEvolutionEvidenceSummary>,
) {
    let Some(incoming) = incoming else {
        return;
    };
