    ...
    match stage {
        ...
        "coordination-gate" | "autonomous-gate" => {
            let decision = metadata
                ...
                .and_then(Value::as_str)
                .map(
                    |status| agendao_orchestrator::SchedulerExecutionGateDecision {
                        status: match status {
                            "done" => agendao_orchestrator::SchedulerExecutionGateStatus::Done,
                            "continue" => {
                                agendao_orchestrator::SchedulerExecutionGateStatus::Continue
                            }
                            _ => agendao_orchestrator::SchedulerExecutionGateStatus::Blocked,
                        },
                        summary: metadata
