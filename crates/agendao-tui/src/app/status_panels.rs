...
impl App {
    ...
    pub(super) fn handle_kill_task(&mut self, id: &str) {
        match agendao_orchestrator::global_lifecycle().cancel_task(id) {
            Ok(()) => {
                self.toast.show(
...
fn tui_session_handoff_mode_label(mode: agendao_types::SessionHandoffMode) -> &'static str {
    match mode {
...
fn tui_tool_repair_kind_summary(counts: &[agendao_types::ToolRepairCount]) -> Option<String> {
    if counts.is_empty() {
        return None;
    }
    Some(
...
fn tui_tool_repair_tool_summary(tools: &[agendao_types::ToolRepairToolSummary]) -> Option<String> {
    if tools.is_empty() {
        return None;
    }
    Some(
...
fn tui_tool_trajectory_band_label(band: agendao_types::ToolTrajectoryQualityBand) -> &'static str {
    match band {
        agendao_types::ToolTrajectoryQualityBand::Clean => "clean",
        agendao_types::ToolTrajectoryQualityBand::Recoverable => "recoverable",
        agendao_types::ToolTrajectoryQualityBand::Degraded => "degraded",
        agendao_types::ToolTrajectoryQualityBand::Risky => "risky",
    }
}
        ...
        if boundary.boundary_recorded {
            ...
            if let Some(continuity) = telemetry.compaction_continuity.as_ref() {
                ...
                continuity_parts.push(match continuity.source {
                    ...
                    agendao_types::SessionCompactionContinuityInspectionSource::RawSummaryFallback => {
    ...
    if let Some(current_tokens) = current_context_tokens {
        ...
        if let Some(limit) = current_context_limit.filter(|limit| *limit > 0) {
            if let Some(percent) = tui_context_usage_percent(current_tokens, limit) {
                if let Some(note) = agendao_types::context_pressure_label(Some(percent)) {
...
fn tui_effective_policy_lines(
    policy: &agendao_types::SessionEffectivePolicyView,
) -> Vec<StatusLine> {
    let mut lines = vec![
...
fn tui_provider_profile_summary(profile: &agendao_types::ProviderProfileDescriptorView) -> String {
    let mut parts = vec![
...
fn tui_scheduler_trace_step_kind_label(
    kind: &agendao_types::SessionEffectiveSchedulerTraceStepKind,
) -> &'static str {
    match kind {
        agendao_types::SessionEffectiveSchedulerTraceStepKind::RequestedProfile => {
            "requested_profile"
        }
        agendao_types::SessionEffectiveSchedulerTraceStepKind::CommandWorkflowOverride => {
            "command_workflow_override"
        }
        agendao_types::SessionEffectiveSchedulerTraceStepKind::SessionPinnedProfile => {
            "session_pinned_profile"
        }
        agendao_types::SessionEffectiveSchedulerTraceStepKind::LegacySessionPinnedProfile => {
            "legacy_session_pinned_profile"
        }
        agendao_types::SessionEffectiveSchedulerTraceStepKind::ConfigDefaultProfile => {
            "config_default_profile"
        }
        agendao_types::SessionEffectiveSchedulerTraceStepKind::AutoRoute => "auto_route",
        agendao_types::SessionEffectiveSchedulerTraceStepKind::SoftFallback => "soft_fallback",
    }
}

fn tui_config_validation_lines(
    snapshot: &agendao_types::ConfigPolicyValidationSnapshot,
) -> Vec<StatusLine> {
    let error_count = snapshot
        .reports
        .iter()
        .filter(|item| item.severity == agendao_types::ConfigPolicyValidationSeverity::Error)
        .count();
    let warning_count = snapshot
        .reports
        .iter()
        .filter(|item| item.severity == agendao_types::ConfigPolicyValidationSeverity::Warning)
        .count();
    ...
    let mut grouped: std::collections::BTreeMap<
        agendao_types::ConfigPolicyValidationOwner,
        Vec<&agendao_types::ConfigPolicyValidationItem>,
    ...
    for (owner, items) in grouped {
        ...
        for item in items {
            ...
            lines.push(match item.severity {
                agendao_types::ConfigPolicyValidationSeverity::Warning => {
                    StatusLine::warning(headline)
                }
                agendao_types::ConfigPolicyValidationSeverity::Error => StatusLine::error(headline),
            });
            lines.push(StatusLine::muted(format!(
                "  Scope: {}",
...
fn tui_config_validation_owner_label(
    owner: agendao_types::ConfigPolicyValidationOwner,
) -> &'static str {
    match owner {
        agendao_types::ConfigPolicyValidationOwner::Scheduler => "Scheduler",
        agendao_types::ConfigPolicyValidationOwner::SkillTree => "Skill Tree",
        agendao_types::ConfigPolicyValidationOwner::ProviderProfile => "Provider Profile",
        agendao_types::ConfigPolicyValidationOwner::ExternalAdapter => "External Adapter",
    }
}

fn tui_config_validation_severity_label(
    severity: agendao_types::ConfigPolicyValidationSeverity,
) -> &'static str {
    match severity {
        agendao_types::ConfigPolicyValidationSeverity::Warning => "warning",
        agendao_types::ConfigPolicyValidationSeverity::Error => "error",
    }
}

fn tui_config_validation_effect_label(
    effect: agendao_types::ConfigPolicyValidationEffect,
) -> &'static str {
    match effect {
        agendao_types::ConfigPolicyValidationEffect::SoftFallback => "soft fallback",
        agendao_types::ConfigPolicyValidationEffect::FailClosedBootstrap => "fail-closed bootstrap",
        agendao_types::ConfigPolicyValidationEffect::FailClosedRequestGate => {
...
fn tui_config_validation_scope_label(
    kind: agendao_types::ConfigPolicyValidationScopeKind,
    subject_id: Option<&str>,
) -> String {
    let base = match kind {
        agendao_types::ConfigPolicyValidationScopeKind::SchedulerPath => "scheduler path",
        agendao_types::ConfigPolicyValidationScopeKind::SkillTree => "skill tree",
        agendao_types::ConfigPolicyValidationScopeKind::Provider => "provider",
        agendao_types::ConfigPolicyValidationScopeKind::ExternalAdapter => "external adapter",
    };
    match subject_id {
...
mod tests {
    ...
    fn usage_surface_explains_raw_history_and_api_view() {
        let telemetry = crate::api::SessionTelemetrySnapshot {
            ...
            tool_repair_summary: Some(agendao_types::SessionToolRepairTelemetrySummary {
                total_tool_calls: 3,
                repaired_tool_call_count: 2,
                error_tool_call_count: 1,
                repair_event_count: 4,
                failure_kinds: vec![agendao_types::ToolRepairCount {
                    key: "invalid_arguments".to_string(),
                    count: 1,
                }],
                provider_diagnostic_count: 1,
                provider_diagnostic_kinds: vec![agendao_types::ToolRepairCount {
                    key: "thinking_replay_rejected".to_string(),
                    count: 1,
                }],
                event_kinds: vec![agendao_types::ToolRepairCount {
                    key: "alias_normalization".to_string(),
                    count: 2,
                }],
                event_layers: vec![agendao_types::ToolRepairCount {
                    key: "tool".to_string(),
                    count: 4,
                }],
                tools: vec![agendao_types::ToolRepairToolSummary {
            ...
            model_tool_repair_summary: Some(agendao_types::ModelToolRepairTelemetrySummary {
            ...
            tool_trajectory_quality: Some(agendao_types::ToolTrajectoryQualitySummary {
                score: 78,
                band: agendao_types::ToolTrajectoryQualityBand::Recoverable,
                total_tool_calls: 3,
                repaired_tool_call_count: 2,
                error_tool_call_count: 1,
                repair_event_count: 4,
            ...
            tool_result_governance: Some(agendao_types::ToolResultGovernanceSummary {
            ...
            ownership: Some(agendao_types::SessionOwnershipSummary {
            ...
            context_compaction_summary: Some(agendao_types::ContextCompactionSummary {
            ...
            context_compaction_lifecycle_summary: Some(
                agendao_types::ContextCompactionLifecycleSummary {
                    trigger: "auto_preflight".to_string(),
                    phase: Some("prompt.pre_request".to_string()),
                    reason: Some("request_view_threshold".to_string()),
            ...
            cache_semantics: Some(agendao_types::SessionCacheSemanticsSummary {
                ...
                prompt_surface_evidence: Some(
                    agendao_types::PromptSurfaceEvidenceSummary {
