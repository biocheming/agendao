...
//!
//! The server exposes agents and scheduler templates as `kind:id` entries.
//! The TUI stores the selected pair and dispatches it to the typed request,
...
pub struct ModeEntry {
    /// `agent` or `scheduler`, matching `ExecutionModeInfo.kind`.
    /// dispatch 处按此分发到 agent 或 scheduler 槽。
    pub kind: String,
    /// Request id, such as `verify` or `reviewer`.
    pub id: String,
    ...
    pub fn render(&self, ctx: &mut RenderContext, geom: backdrop::PromptGeom) {
        ...
        let items: Vec<ListItem> = self
            ...
            .map(|(i, e)| {
                ...
                // Prefix the kind so agents and schedulers remain easy to scan.
...
mod tests {
    ...
    fn composite_formats_kind_id() {
        assert_eq!(entry("scheduler", "verify").composite(), "scheduler:verify");
    ...
    fn enter_returns_selected_and_closes() {
        ...
        d.open_with(vec![entry("scheduler", "direct"), entry("agent", "b")]);
    ...
    fn down_wraps_around() {
        ...
        d.open_with(vec![
            ...
            entry("scheduler", "verify"),
        ]);
        let _ = d.handle_key(&Key::Down);
        let _ = d.handle_key(&Key::Down); // wrap back to 0
        let picked = d.handle_key(&Key::Enter).unwrap();
        assert_eq!(picked.id, "direct");
    }

    #[test]
    fn esc_closes_without_selection() {
        let mut d = ModeSelectDialog::new();
        d.open_with(vec![entry("scheduler", "direct")]);
