...
impl QuestionRequest {
    ...
    pub fn from_info(info: &agendao_client::QuestionInfo) -> QuestionRequest {
        let item = info.items.first();
        let qtext = item.map(|item| item.question.clone()).unwrap_or_default();
        let options = item
            .map(|item| {
                item.options
                    .iter()
...
impl QuestionDialog {
    ...
    pub fn ask(&mut self, q: QuestionRequest) {
        ...
        if self.requests.iter().any(|r| r.id == q.id) {
            return;
        }
        let n = q.options.len();
    ...
    pub fn handle_key(&mut self, key: &Key) -> Option<QuestionKeyOutcome> {
        ...
        match key {
            ...
            Key::Char(' ') => {
                if let Some(t) = self.toggled.get_mut(self.selected) {
                    *t = !*t;
                }
                None
            }
    ...
    pub fn render_inline(&self) -> Option<BlockLayout> {
        ...
        let hint = if is_multi {
            "Space toggle · Enter confirm · s skip · Esc hide"
        } else {
            "↑↓ choose · Enter select · s skip · Esc hide"
        };
        ...
        for (i, opt) in req.options.iter().enumerate() {
            let marker = if is_multi {
                if self.toggled.get(i).copied().unwrap_or(false) {
                    "☑ "
                } else {
                    "☐ "
                }
            ...
            content = content.child_sized(Text::new(format!("{}{}", marker, label)).fg(color), 1);
            height += 1;
        }

        content = content.child_sized(Text::new(format!(" {}", hint)).fg(colors::FG_MUTED()), 1);
        height += 1;
