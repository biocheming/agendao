...
fn ensure_embedded_web_dist() -> Result<(), String> {
    ...
    let repo_root = manifest_dir
        ...
        .to_path_buf();
    let web_root = repo_root.join("apps").join("agendao-web");
    ...
    println!("cargo:rerun-if-env-changed=AGENDAO_WEB_DIST");
    println!("cargo:rerun-if-env-changed=AGENDAO_WEB_SKIP_BUILD");

    if env::var_os("AGENDAO_WEB_SKIP_BUILD").is_some() {
        println!("cargo:warning=AGENDAO_WEB_SKIP_BUILD set; skipping automatic agendao-web build");
        return Ok(());
    }
    ...
    let npm = find_npm().ok_or_else(|| {
        format!(
            "agendao-web assets are stale or missing at `{}` and `npm` was not found in PATH",
            dist_root.display()
        )
    })?;

    println!(
        "cargo:warning=building embedded agendao-web assets with `{}` in {}",
        npm.display(),
        web_root.display()
    );
    ...
    if !status.success() {
        return Err(format!(
            "`{}` failed with status {} while building apps/agendao-web",
            npm.display(),
            status
        ));
    }
