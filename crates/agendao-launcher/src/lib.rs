...
pub fn resolve_web_dist_dir() -> anyhow::Result<PathBuf> {
    ...
    anyhow::bail!(
        "AgenDao Web frontend assets were not found. By default AgenDao expects embedded web assets; for external assets, build `apps/agendao-web` and set `AGENDAO_WEB_DIST`, or install a package that includes `share/agendao/web`."
    )
}

pub fn try_resolve_web_dist_override() -> Option<PathBuf> {
    let value = std::env::var("AGENDAO_WEB_DIST").ok()?;
...
pub fn resolve_web_dev_url() -> anyhow::Result<Option<Url>> {
    let Ok(value) = std::env::var("AGENDAO_WEB_DEV_URL") else {
        return Ok(None);
    };

    let trimmed = value.trim();
    if trimmed.is_empty() {
        anyhow::bail!("AGENDAO_WEB_DEV_URL is set but empty");
    }

    let url = Url::parse(trimmed)
        .with_context(|| format!("Failed to parse AGENDAO_WEB_DEV_URL `{trimmed}`"))?;
    match url.scheme() {
        "http" | "https" => Ok(Some(url)),
        other => anyhow::bail!(
            "AGENDAO_WEB_DEV_URL must use http or https, got scheme `{}`",
            other
        ),
    }
}
...
pub fn resolve_desktop_workspace(app_name: &str) -> anyhow::Result<PathBuf> {
    ...
    anyhow::bail!(
        "Could not determine a workspace for desktop launch. Start with `agendao web --dir <path>` or launch from inside a project directory."
    );
}
