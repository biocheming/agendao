...
use rocode_config::{Config, ConfigLoader, ProviderConfig};
use rocode_provider::{
    export_provider_artifact_bundle, import_provider_artifact_bundle,
    ConfigProvider as BootstrapConfigProvider,
};
use rocode_types::{ProviderArtifactBundle, ProviderArtifactImportEnvelope};
...
fn import_provider_artifact_into_project_config(
    ...
    rocode_config::write_config(base_dir, &config)?;
...
mod tests {
    ...
    use rocode_config::{Config, ProviderConfig};
    use rocode_types::{
    ...
    fn write_config(root: &Path, content: &str) {
        fs::write(root.join("rocode.json"), content).expect("config should be written");
    }
    ...
    fn import_provider_artifact_preserves_existing_api_key_and_replaces_core_fields() {
        ...
        let imported = rocode_provider::import_provider_artifact_bundle(
            rocode_types::ProviderArtifactImportEnvelope::Bundle(sample_bundle()),
        )
  ...
  "provider": {
    "openai": {
      "api_key": "keep-me"
        ...
        assert!(exported.contains("\"version\": \"rocode-rust/provider/v1\""));
        assert!(!exported.contains("secret-123"));

        let imported = import_provider_data_into_dir(&target, &export_path).expect("import");
        assert_eq!(imported, 1);

        let config = fs::read_to_string(target.join("rocode.json")).expect("persisted config");
