...
pub(in crate::run) fn cli_session_handoff_mode_label(
    mode: rocode_types::SessionHandoffMode,
) -> &'static str {
    match mode {
...
pub(in crate::run) fn extend_wrapped_lines(out: &mut Vec<String>, text: &str, width: usize) {
    ...
    let wrapped = rocode_command::cli_panel::wrap_display_text(text, width.max(1));
    if wrapped.is_empty() {
        out.push(String::new());
    } else {
        out.extend(wrapped);
...
pub(in crate::run) fn cli_active_stage_context_lines(
    ...
    let mut lines = vec![
        rocode_command::cli_panel::truncate_display(&header, max_width),
        rocode_command::cli_panel::truncate_display(
            &format!("Status: {}", summary.join(" · ")),
            max_width,
        ),
    ];
    if let Some(focus) = stage.focus.as_deref().filter(|value| !value.is_empty()) {
        lines.push(rocode_command::cli_panel::truncate_display(
            &format!("Focus: {focus}"),
            max_width,
        ));
    }
