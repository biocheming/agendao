...
use rocode_client::FrontendTransport;
use rocode_client::PromptPart;
...
pub async fn send_prompt(
    ...
    if let Some(state) = local_server {
        ...
        let response = transport
            ...
            .await?;
        Ok(rocode_client::PromptResponse {
...
pub async fn send_command_prompt(
    ...
    if let Some(state) = local_server {
        return rocode_server::local_prompt(
            Arc::clone(state),
            session_id,
            rocode_client::PromptRequest {
                message: None,
...
pub async fn get_session(
    ...
    api_client: &CliApiClient,
    session_id: &str,
) -> anyhow::Result<rocode_client::SessionInfo> {
    if let Some(state) = local_server {
        return rocode_server::local_get_session(Arc::clone(state), session_id).await;
    }
...
pub async fn list_messages(
    ...
    api_client: &CliApiClient,
    session_id: &str,
) -> anyhow::Result<Vec<rocode_client::MessageInfo>> {
    if let Some(state) = local_server {
        return rocode_server::local_list_messages(Arc::clone(state), session_id, None, None).await;
    }
...
pub async fn list_questions(
    local_server: &Option<Arc<rocode_server::ServerState>>,
    api_client: &CliApiClient,
) -> anyhow::Result<Vec<rocode_client::QuestionInfo>> {
    if let Some(state) = local_server {
        return rocode_server::local_list_questions(Arc::clone(state)).await;
    }
...
pub async fn reply_question(
    ...
    if let Some(state) = local_server {
        return rocode_server::local_reply_question(Arc::clone(state), question_id, answers).await;
    }
...
pub async fn reject_question(
    ...
    if let Some(state) = local_server {
        return rocode_server::local_reject_question(Arc::clone(state), question_id).await;
    }
...
pub async fn reply_permission(
    ...
    if let Some(state) = local_server {
        return rocode_server::local_reply_permission(
            Arc::clone(state),
            permission_id,
            reply.to_string(),
            message,
...
pub async fn get_workspace_context(
    ...
    if let Some(state) = local_server {
        return rocode_server::local_get_workspace_context(Arc::clone(state)).await;
    }
...
pub async fn list_sessions(
    ...
    search: Option<&str>,
    limit: Option<usize>,
) -> anyhow::Result<Vec<rocode_client::SessionListItem>> {
    if let Some(state) = local_server {
        return rocode_server::local_list_sessions(
            Arc::clone(state),
...
pub async fn get_all_providers(
    ...
    transport: &Option<Arc<FrontendTransport>>,
    api_client: &CliApiClient,
) -> anyhow::Result<rocode_client::FullProviderListResponse> {
    if let Some(state) = local_server {
        return rocode_server::local_get_all_providers(Arc::clone(state)).await;
    }
    if let Some(transport) = transport {
...
pub async fn get_recent_models(
    ...
    if let Some(state) = local_server {
        return rocode_server::local_get_recent_models(Arc::clone(state)).await;
    }
    if let Some(transport) = transport {
...
pub async fn put_recent_models(
    ...
    if let Some(state) = local_server {
        return rocode_server::local_put_recent_models(
            Arc::clone(state),
...
pub async fn list_execution_modes(
    ...
    transport: &Option<Arc<FrontendTransport>>,
    api_client: &CliApiClient,
) -> anyhow::Result<Vec<rocode_client::ExecutionModeInfo>> {
    if let Some(state) = local_server {
        return rocode_server::local_list_execution_modes(Arc::clone(state)).await;
    }
    if let Some(transport) = transport {
...
pub async fn list_agents(
    ...
    transport: &Option<Arc<FrontendTransport>>,
    api_client: &CliApiClient,
) -> anyhow::Result<Vec<rocode_client::AgentInfo>> {
    if let Some(state) = local_server {
        return rocode_server::local_list_agents(Arc::clone(state)).await;
    }
    if let Some(transport) = transport {
