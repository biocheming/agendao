...
mod tests {
    ...
    use rocode_skill::SkillAuthority;
    use std::fs;
    use std::path::{Path, PathBuf};
    use std::sync::Arc;
    ...
    fn write_workspace_skill(
        ...
        let skill_dir = root.join(".rocode/skills").join(relative_dir);
    ...
    fn authority_for(base_dir: &Path) -> SkillAuthority {
        let config_store = rocode_config::ConfigStore::from_project_dir(base_dir)
            .ok()
            .map(Arc::new);
        SkillAuthority::new(base_dir.to_path_buf(), config_store)
    }
    ...
    fn workspace_skill_export_and_import_roundtrip_through_cli_helpers() {
        ...
        assert!(exported.contains("\"version\": \"rocode-rust/workspace-skill/v1\""));
