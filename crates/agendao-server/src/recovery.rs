...
fn assistant_visible_text_from_message(message: &agendao_session::SessionMessage) -> String {
    let mut out = String::new();
    for part in &message.parts {
        if let agendao_session::PartType::Text { text, ignored, .. } = &part.part_type {
...
fn latest_assistant_finish(session: &agendao_session::Session) -> Option<String> {
    session
...
pub(crate) fn collect_subtask_recovery_targets(
    ...
    for message in session.record().messages.iter().rev() {
        ...
        if !matches!(message.role, agendao_session::MessageRole::User) {
            continue;
        }
        for part in message.parts.iter().rev() {
            if let agendao_session::PartType::Subtask {
                id,
                description,
                status,
            } = &part.part_type
...
mod tests {
    ...
    fn recovery_protocol_surfaces_run_stage_and_subtask_actions() {
        let mut session = agendao_session::Session::new("default", ".");
    ...
    fn recovery_protocol_surfaces_abort_actions_while_running() {
        let session = agendao_session::Session::new("proj", "/tmp");
