...
fn filtered_tool_definitions(
    ...
    agendao_session::prioritize_tool_definitions(&mut tool_defs);
    tool_defs
}
...
fn stream_ingress_envelope(
    ...
    let mut envelope = agendao_session::prompt::IngressTurnEnvelope::new_text(
        ...
        agendao_session::prompt::normalize_ingress_source(ingress_source),
        turn_id,
        now,
        text.to_string(),
    );
...
) -> std::result::Result<Sse<impl Stream<Item = std::result::Result<Event, Infallible>>>, ApiError>
{
    ...
    tokio::spawn(async move {
        ...
        let prompt_runner = agendao_session::SessionPrompt::new(Arc::new(RwLock::new(
            agendao_session::SessionStateManager::new(),
        )))
        .with_config_store(stream_state.config_store.clone())
        .with_tool_runtime_config(agendao_tool::ToolRuntimeConfig::from_config(&stream_config));
