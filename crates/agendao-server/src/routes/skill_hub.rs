...
fn build_skill_hub_install_permission_request(
    source: &agendao_types::SkillSourceRef,
    skill_name: &str,
) -> PermissionRequest {
...
mod tests {
    ...
    fn write_workspace_skill(project_dir: &std::path::Path, relative_dir: &str, content: &str) {
        let skill_dir = project_dir.join(".agendao/skills").join(relative_dir);
...
"#,
    ...
    async fn run_skill_guard_returns_report_for_existing_skill() {
        let dir = tempdir().expect("tempdir");
        fs::create_dir_all(dir.path().join(".agendao/skills/guarded")).expect("skill dir");
        fs::write(
            dir.path().join(".agendao/skills/guarded/SKILL.md"),
            r#"---
name: guarded
description: guarded
---
...
"#,
    ...
    async fn governance_timeline_returns_managed_and_guard_entries() {
        let dir = tempdir().expect("tempdir");
        fs::create_dir_all(dir.path().join(".agendao/skills/timeline")).expect("skill dir");
        fs::write(
            dir.path().join(".agendao/skills/timeline/SKILL.md"),
            r#"---
name: timeline
...
"#,
    ...
    async fn usage_ledger_returns_operational_snapshots() {
        let dir = tempdir().expect("tempdir");
        fs::create_dir_all(dir.path().join(".agendao/skills/frontend-ui-ux")).expect("skill dir");
        fs::write(
            dir.path().join(".agendao/skills/frontend-ui-ux/SKILL.md"),
            r#"---
name: frontend-ui-ux
...
"#,
    ...
    async fn negative_entropy_route_returns_usage_backed_candidates() {
        ...
        assert!(entry
            .signals
            .contains(&agendao_types::SkillNegativeEntropySignal::NeverReused));
    }
...
"#,
        ...
        let redundant = response
            ...
            .iter()
            .find(|edge| {
                edge.relation_kind == agendao_types::SkillRelationshipKind::RedundantOverlap
...
"#,
        ...
        let family = response
            ...
            .iter()
            .find(|group| {
                group.group_kind == agendao_types::SkillCapabilityGroupKind::CanonicalFamily
        ...
        assert!(family.members.iter().any(|member| {
            member.skill_name == "code-review"
                && member.role == agendao_types::SkillCapabilityMemberRole::MergeCandidate
        }));
        ...
        let bundle = response
            ...
            .iter()
            .find(|group| {
                group.group_kind == agendao_types::SkillCapabilityGroupKind::ComplementaryBundle
            })
...
"#,
        ...
        assert!(inspection.relationships.iter().any(|edge| {
            edge.relation_kind == agendao_types::SkillRelationshipKind::RedundantOverlap
                && edge.preferred_skill_name.as_deref() == Some("repo-review")
                && edge.state == agendao_types::SkillRelationshipState::Accepted
        }));
...
"#,
        ...
        let Json(created) = activate_composition_group(
            State(state.clone()),
            Json(agendao_types::SkillHubCompositionGroupCreateRequest {
                session_id: session_id.to_string(),
                ...
                members: vec![
                    agendao_types::SkillCapabilityMember {
                        skill_name: "repo-review".to_string(),
                        role: agendao_types::SkillCapabilityMemberRole::Canonical,
                    },
                    agendao_types::SkillCapabilityMember {
                        skill_name: "code-review".to_string(),
                        role: agendao_types::SkillCapabilityMemberRole::MergeCandidate,
                    },
                ],
        ...
        let Json(updated) = set_composition_group_member_role(
            State(state.clone()),
            Json(agendao_types::SkillHubCompositionGroupMemberRoleRequest {
                session_id: session_id.to_string(),
                capability_id: "cap_review_family".to_string(),
                skill_name: "security-review".to_string(),
                role: agendao_types::SkillCapabilityMemberRole::Specialization,
            }),
        )
        .await
        ...
        assert!(updated.group.members.iter().any(|member| {
            member.skill_name == "security-review"
                && member.role == agendao_types::SkillCapabilityMemberRole::Specialization
        }));

        let Json(removed) = remove_composition_group_member(
            State(state.clone()),
            Json(agendao_types::SkillHubCompositionGroupMemberRemoveRequest {
                session_id: session_id.to_string(),
                capability_id: "cap_review_family".to_string(),
                skill_name: "code-review".to_string(),
            }),
        ...
        assert!(timeline.entries.iter().any(|entry| {
            entry.kind
                == agendao_types::SkillGovernanceTimelineKind::CapabilityGroupMemberRoleUpdated
        }));
        PERMISSION_ENGINE.lock().await.clear_session(session_id);
    }
    ...
    async fn semantic_conflict_route_returns_ledger_prioritized_pairs() {
        let dir = tempdir().expect("tempdir");
        let skills_root = dir.path().join(".agendao/skills");
...
"#,
    ...
    async fn plan_remote_install_returns_install_entry_for_registry_source() {
        ...
        assert_eq!(
            ...
            agendao_types::SkillRemoteInstallAction::Install
        );
        assert_eq!(response.distribution.skill_name, "remote-reviewer");
        assert_eq!(
            response.distribution.lifecycle,
            agendao_types::SkillManagedLifecycleState::Resolved
        );
    }
    ...
    async fn plan_and_apply_remote_update_use_same_lifecycle_contract() {
        ...
        assert_eq!(
            ...
            agendao_types::SkillManagedLifecycleState::UpdateAvailable
        );
        ...
        assert_eq!(
            response.plan.entry.action,
            agendao_types::SkillRemoteInstallAction::Update
        );
        assert!(std::path::Path::new(&response.result.location).exists());
    ...
    async fn detach_and_remove_managed_skill_routes_expose_results() {
        ...
        assert_eq!(
            ...
            agendao_types::SkillManagedLifecycleState::Detached
        );
        let authority = skill_governance_authority(&state);
        assert!(authority
            .skill_authority()
