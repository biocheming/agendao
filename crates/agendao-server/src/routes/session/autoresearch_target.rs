...
fn discover_workspace_targets(workspace_root: &Path) -> Vec<WorkspaceAutoresearchTarget> {
    let skills_root = workspace_root.join(".agendao").join("skills");
...
mod tests {
    ...
    fn write_skill(root: &Path, dir_name: &str, skill_markdown: &str, verify: bool, guard: bool) {
        let skill_root = root.join(".agendao").join("skills").join(dir_name);
...
"#
    ...
    fn resolve_autoresearch_command_resolves_unique_workspace_target() {
        ...
        assert_eq!(
            ...
            Some("bash .agendao/skills/coverage-lift/scripts/verify.sh")
        );
        assert!(override_record
            ...
            .map(|objective| objective
                .scope
                .exclude
                .contains(&".agendao/skills/coverage-lift/**".to_string()))
            .unwrap_or(false));
    }
