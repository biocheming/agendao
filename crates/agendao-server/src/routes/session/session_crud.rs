...
fn validate_session_revert_target(
    ...
    if agendao_session::Session::is_imported_fork_history_message(message) {
...
fn session_time_info(session: &agendao_session::Session) -> SessionTimeInfo {
    let session = session.record();
    agendao_types::SessionTime {
...
fn session_summary_info(session: &agendao_session::Session) -> Option<SessionSummaryInfo> {
    let session = session.record();
...
fn session_list_time(session: &agendao_session::Session) -> agendao_types::SessionTime {
    let session = session.record();
    agendao_types::SessionTime {
        created: session.time.created,
        updated: session.time.updated,
...
fn session_list_summary(session: &agendao_session::Session) -> Option<SessionListSummary> {
    let session = session.record();
    session.summary.as_ref().map(|s| SessionListSummary {
        additions: s.additions,
        deletions: s.deletions,
...
fn session_list_contract() -> SessionListContract {
    SessionListContract {
        ...
        search_fields: agendao_session::SESSION_LIST_SEARCH_FIELDS
            .iter()
...
pub(super) fn session_to_list_item(session: &agendao_session::Session) -> SessionListItem {
...
pub(crate) fn session_to_info(session: &agendao_session::Session) -> SessionInfo {
    let session_record = session.record();
    SessionInfo {
        id: session_record.id.clone(),
        slug: session_record.slug.clone(),
        ...
        share: session_record
            .share
            .as_ref()
            .map(|s| agendao_types::SessionShare { url: s.url.clone() }),
...
pub(super) fn session_model_override(session: &agendao_session::Session) -> Option<String> {
    session
        .record()
        .metadata
        .get("model_provider")
...
pub(super) fn session_variant_override(session: &agendao_session::Session) -> Option<String> {
    session
        .record()
        .metadata
        .get("model_variant")
...
pub(super) fn session_agent_override(session: &agendao_session::Session) -> Option<String> {
    session
        .record()
        .metadata
        .get("agent")
...
pub(super) async fn list_sessions(
    ...
    let filter = agendao_session::SessionFilter {
    ...
    let status: HashMap<String, SessionStatusInfo> = sessions
        ...
        .map(|s| {
            let lifecycle_status = match s.status {
                agendao_session::SessionStatus::Active => "active",
                agendao_session::SessionStatus::Completed => "completed",
                agendao_session::SessionStatus::Archived => "archived",
                agendao_session::SessionStatus::Compacting => "compacting",
            };
            ...
            let (status, idle, busy, attempt, message, next) = match run {
                ...
                SessionRunStatus::Blocked { reason, .. } => {
                    ("blocked".to_string(), false, false, None, reason, None)
                }
                SessionRunStatus::Sleeping { reason, .. } => {
                    ("sleeping".to_string(), false, false, None, reason, None)
                }
            };
            (
                s.id.clone(),
...
mod tests {
    ...
    fn session_to_info_surfaces_typed_fork_contract() {
        ...
        assert_eq!(
            ...
            agendao_types::SessionForkLifecycleScope::ForkPromptSurface
        );
    }
    ...
    async fn start_compaction_route_reports_skipped_when_history_is_too_small() {
        ...
        assert_eq!(
            response
                ...
                .as_ref()
                .map(|lifecycle| lifecycle.status),
            Some(agendao_types::ContextCompactionLifecycleStatus::Skipped)
        );
...
pub(super) async fn set_session_summary(
    ...
    let updated = sessions
        .set_summary(
            &id,
            agendao_session::SessionSummary {
                ...
                diffs: req.diffs.map(|diffs| {
                    diffs
                        .into_iter()
                        .map(|d| agendao_session::FileDiff {
                            path: d.path,
...
pub(super) async fn update_part(
    ...
    let mut part: agendao_session::MessagePart = serde_json::from_value(req.part)
    ...
    // Verify the tool call exists in the session (hold lock briefly).
    {
        ...
        let found = session.messages.iter().any(|msg| {
            msg.parts.iter().any(|part| {
                matches!(
                    &part.part_type,
                    agendao_session::PartType::ToolCall { id, .. } if id == &tool_call_id
                )
            })
        });
    ...
    if let Some(tracking) = agendao_plugin::subprocess::get_tool_call_tracking(&tool_call_id).await {
        ...
        if let Some(loader) = super::super::get_plugin_loader() {
                ...
                .find(|c| c.plugin_id() == tracking.plugin_name)
            {
                ...
                agendao_plugin::subprocess::remove_tool_call_tracking(&tool_call_id).await;
