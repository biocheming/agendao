...
fn scheduler_ingress_envelope(
    ...
    let mut envelope = agendao_session::prompt::IngressTurnEnvelope::new_text(
        ...
        agendao_session::prompt::IngressSource::Scheduler,
    ...
    envelope.stabilization.policy =
        agendao_session::prompt::INGRESS_POLICY_SCHEDULER_METADATA_ONLY.to_string();
    envelope.source_origin = Some(agendao_types::MessageSourceOrigin::Scheduler);
    envelope
}
...
impl AgentResolver for SchedulerAgentResolver {
    fn resolve(&self, name: &str) -> Option<agendao_orchestrator::AgentDescriptor> {
        self.registry
            .get(name)
...
fn to_orchestrator_agent_descriptor(info: &AgentInfo) -> agendao_orchestrator::AgentDescriptor {
    agendao_orchestrator::AgentDescriptor {
        name: info.name.clone(),
        system_prompt: info.system_prompt.clone(),
        model: info
            .model
            .as_ref()
            .map(|model| agendao_orchestrator::ModelRef {
...
pub(crate) fn to_task_agent_info(info: &AgentInfo) -> agendao_tool::TaskAgentInfo {
    agendao_tool::TaskAgentInfo {
        name: info.name.clone(),
        model: info.model.as_ref().map(|m| agendao_tool::TaskAgentModel {
...
impl ModelResolver for SessionSchedulerModelResolver {
    ...
    ) -> std::result::Result<agendao_provider::StreamResult, OrchestratorError> {
...
impl agendao_orchestrator::runtime::events::CancelToken for SchedulerRunCancelToken {
        ...
        .with_resolve_category({
            ...
            move |category| {
                ...
                async move {
                    Ok(registry
                        .resolve(&category)
                        .map(|def| agendao_tool::TaskCategoryInfo {
                            name: category,
                            description: def.description.clone(),
                            model: def.model.as_ref().map(|m| agendao_tool::TaskAgentModel {
                                provider_id: m.provider_id.clone(),
                                model_id: m.model_id.clone(),
                            }),
...
impl OrchestratorToolExecutor for SessionSchedulerToolExecutor {
    async fn execute(
        ...
        let result = self
            ...
            .map_err(|error| match error {
                ...
                agendao_tool::ToolError::PermissionDenied(message) => {
                    OrchestratorToolExecError::PermissionDenied(message)
                }
                agendao_tool::ToolError::Cancelled => {
    ...
    async fn list_definitions(
        ...
        agendao_session::prioritize_tool_definitions(&mut tools);
        tools
    }
}
...
async fn resolve_local_scheduler_prompt_parts(
    ...
    let known_agents = AgentRegistry::from_config(config)
        ...
        .collect::<Vec<_>>();
    agendao_session::resolve_prompt_parts(prompt_text, Path::new(directory), &known_agents).await
}
...
mod tests {
    ...
    fn scheduler_ingress_envelope_uses_internal_scheduler_contract() {
        ...
        assert_eq!(
            ...
            agendao_session::prompt::INGRESS_POLICY_SCHEDULER_METADATA_ONLY
        );
    }
    ...
    async fn local_scheduler_prompt_parts_resolve_file_references() {
        let temp_dir =
            std::env::temp_dir().join(format!("agendao-local-scheduler-{}", uuid::Uuid::new_v4()));
        ...
        assert!(matches!(
            &parts[0],
            agendao_session::prompt::PartInput::Text { text } if text == "Inspect @note.txt"
        ));
        assert!(parts.iter().any(|part| matches!(
            part,
            agendao_session::prompt::PartInput::File { filename, mime, .. }
