...
use agendao_orchestrator::runtime::StepCheckpointDirective;
...
struct ActiveStageMessage {
    ...
    stage_name: String,
    step_count: u32,
    committed_usage: agendao_orchestrator::runtime::events::StepUsage,
    live_usage: agendao_orchestrator::runtime::events::StepUsage,
...
impl SessionSchedulerLifecycleHook {
    ...
    async fn emit_stage_block(&self, message: &SessionMessage) {
        ...
        if let Some(block) = scheduler_stage_block_from_message(message) {
            ...
            let stage_identity = agendao_types::LiveMessagePartIdentity {
                message_id: message.id.clone(),
                part_key: agendao_types::scheduler_stage_part_key(&stage_id),
        ...
        match block {
            OutputBlock::Message(message) if message.role == OutputMessageRole::Assistant => {
                ...
                let phase = match message.phase {
                    MessagePhase::Start => agendao_types::LivePartPhase::Start,
                    MessagePhase::Delta => agendao_types::LivePartPhase::Append,
                    MessagePhase::Full => agendao_types::LivePartPhase::Snapshot,
                    MessagePhase::End => agendao_types::LivePartPhase::End,
                };
    ...
    async fn resolve_step_checkpoint_provider(
        &self,
    ) -> Result<Option<(Arc<dyn Provider>, String)>, agendao_orchestrator::OrchestratorError> {
        ...
        let provider = providers.get_provider(&provider_id).map_err(|error| {
            agendao_orchestrator::OrchestratorError::from_provider_error(
                &provider_id,
                Some(&model_id),
                &error,
            )
    ...
    async fn govern_step_checkpoint(
        ...
        Option<agendao_types::ContextPressureGovernanceSummary>,
        agendao_orchestrator::OrchestratorError,
    > {
        ...
        let summary = {
            ...
            let summary = agendao_session::prompt::assess_request_view_context_governance(
                provider.as_ref(),
                &model_id,
                None,
            ...
            agendao_session::prompt::record_context_pressure_governance_summary(
                &mut session,
                &summary,
            );
            sessions.update(session);
...
fn format_step_checkpoint_context_pressure_error(
    summary: &agendao_types::ContextPressureGovernanceSummary,
) -> String {
    let reason = summary
        .reason
        .as_deref()
...
fn directive_from_context_pressure_summary(
    ...
    checkpoint: &agendao_orchestrator::runtime::events::StepCheckpointSnapshot,
...
fn preferred_checkpoint_request_context_tokens(
    checkpoint: &agendao_orchestrator::runtime::events::StepCheckpointSnapshot,
    usage: &agendao_orchestrator::runtime::events::StepUsage,
) -> Option<u64> {
...
impl LifecycleHook for SessionSchedulerLifecycleHook {
    ...
    async fn on_step_start(
        ...
        self.update_active_stage_message(
            |message, active| {
                active.step_count += 1;
                active.live_usage = agendao_orchestrator::runtime::events::StepUsage::default();
    ...
    async fn on_orchestration_end(&self, _: &str, _: u32, _: &OrchestratorExecutionContext) {
        ...
        let result: Option<()> = async {
            ...
            let summary_diffs: Vec<agendao_session::summary::SummaryFileDiff> = file_diffs
                .iter()
                .map(|d| agendao_session::summary::SummaryFileDiff {
                    file: agendao_session::summary::unquote_git_path(&d.path),
                    additions: d.additions,
                    deletions: d.deletions,
                })
                .collect();

            let summary_data = agendao_session::summary::SessionSummaryData {
            ...
            agendao_session::summary::set_session_summary(&mut session, &summary_data);
            let _ = agendao_session::summary::persist_session_diffs(
                &mut session,
                &session_id,
                &summary_data.diffs,
            );
    ...
    async fn on_scheduler_stage_start(
        ...
        self.active_stage_messages
            ...
            .push(ActiveStageMessage {
                ...
                step_count: 0,
                committed_usage: agendao_orchestrator::runtime::events::StepUsage::default(),
                live_usage: agendao_orchestrator::runtime::events::StepUsage::default(),
                attached_session_id,
                attached_message_id,
    ...
    async fn on_step_checkpoint(
        ...
        _exec_ctx: &OrchestratorExecutionContext,
    ) -> Result<Option<StepCheckpointDirective>, agendao_orchestrator::OrchestratorError> {
...
fn extract_todo_items_from_args(
    tool_args: &serde_json::Value,
) -> Option<Vec<agendao_session::TodoInfo>> {
    let todos = tool_args.get("todos")?.as_array()?;
    if todos.is_empty() {
        return None;
    }
...
fn route_outcome_field(decision: &RouteDecision) -> (String, &'static str) {
    match decision.mode {
        ...
        agendao_orchestrator::RouteMode::Orchestrate => ("Orchestrate".to_string(), "success"),
    }
}
...
pub fn assistant_visible_text(message: &SessionMessage) -> String {
    ...
    agendao_session::sanitize_display_text(&out)
}
...
pub fn visible_assistant_text_from_orchestrator_output(output: &str) -> String {
    ...
    if let Some(decision) = parse_route_decision(trimmed) {
        if matches!(decision.mode, agendao_orchestrator::RouteMode::Direct) {
            if let Some(response) = decision
                ...
                .filter(|value| !value.is_empty())
            {
                return agendao_session::sanitize_display_text(response);
            }
        }
    }

    agendao_session::sanitize_display_text(output)
}
...
pub(crate) fn first_user_message_text(session: &Session) -> Option<String> {
    session
        ...
        .map(|text| agendao_session::sanitize_session_title_source(&text))
