...
//!
//! Design constraints (from the AgenDao Constitution):
...
mod tests {
    ...
    async fn sleeping_state_round_trip() {
        ...
        store
            .mark_sleeping(
                "ses_sleep",
                Some("paused until morning".to_string()),
                Some(9000),
            )
            .await;
    ...
    async fn blocked_and_stage_blocked_are_separate_concepts() {
        ...
        // affect StageStatus (which lives in agendao-content stage_protocol).
        let store = RuntimeStateStore::new();
        store
        ...
        // agendao_content::stage_protocol — they share a name but have different
