...
async fn handle_prompt<S: SessionStore + Send + 'static>(
    ...
    // now uses the same SessionManager as ServerState).
    {
        ...
        if sessions.get_mut(&req.session_id).is_none() {
            sessions.create(
                &req.session_id,
                state.workspace_root.to_string_lossy().as_ref(),
            );
        }
    }
...
async fn handle_list_sessions(state: &Arc<ServerState>) -> Result<serde_json::Value, JsonRpcError> {
    ...
    let items: Vec<SessionListItem> = sessions
        ...
        .map(|s| {
            ...
            SessionListItem {
                ...
                last_message: record.messages.last().map(|m| {
                    m.parts
                        .iter()
                        .filter_map(|p| {
