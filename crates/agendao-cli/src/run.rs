...
use agendao_orchestrator::SchedulerRequestDefaults;
...
fn print_assistant_messages(messages: &[agendao_client::MessageInfo]) {
    for msg in messages {
        if msg.role != "user" {
...
mod tests {
    ...
    fn interactive_live_assistant_stream_keeps_single_header_across_tool_cycles_and_full_chunks() {
        ...
        let tool_start_identity = live_identity(
            "assistant-final",
            &agendao_types::tool_call_part_key("tool-1"),
        ...
        let tool_done_identity = live_identity(
            "assistant-final",
            &agendao_types::tool_result_part_key("tool-1"),
    ...
    fn interactive_scheduler_stage_identity_does_not_enter_live_slot_transcript() {
        ...
        handle_sse_event(
            ...
            output_block_event(
                ...
                Some(live_identity(
                    "assistant-1",
                    &agendao_types::scheduler_stage_part_key("stage-1"),
    ...
    fn interactive_tool_call_identity_does_not_enter_root_transcript() {
        ...
        handle_sse_event(
            ...
            output_block_event(
                ...
                Some(live_identity(
                    "assistant-1",
                    &agendao_types::tool_call_part_key("tool-call-1"),
                    LiveMessagePartKind::ToolCall,
                    LivePartPhase::Snapshot,
                    Some("tool-call-1"),
                )),
    ...
    fn interactive_tool_result_identity_enters_root_transcript_with_detail() {
        ...
        handle_sse_event(
            ...
            output_block_event(
                ...
                Some(live_identity(
                    "assistant-1",
                    &agendao_types::tool_result_part_key("tool-call-1"),
                    LiveMessagePartKind::ToolResult,
                    LivePartPhase::End,
                    Some("tool-call-1"),
                )),
    ...
    fn live_tool_snapshot_replaces_same_slot_without_prefix_replay() {
        ...
        let mut transcript = CliVisibleTranscript::default();
        let slot_key = agendao_types::live_slot_key(
            "assistant-1",
            &agendao_types::tool_result_part_key("tool-1"),
        );
    ...
    fn startup_banner_uses_recent_session_metadata() {
        ...
        assert!(banner.contains("AgenDao"));
    ...
    fn retained_layout_emits_session_messages_sidebar_and_active_boxes() {
        ...
        assert!(joined.contains("AgenDao"));
