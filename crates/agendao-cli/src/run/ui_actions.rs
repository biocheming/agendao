...
pub(super) enum CliModelCommand {
    ...
    Add {
        provider_id: String,
        model_key: String,
        config: agendao_config::ModelConfig,
    },
    Edit {
        provider_id: String,
        model_key: String,
...
pub(super) fn cli_apply_model_config_assignments(
    config: &mut agendao_config::ModelConfig,
    assignments: &[&str],
) -> anyhow::Result<()> {
...
pub(super) fn cli_parse_model_command(
    argument: Option<&str>,
    current_config: Option<&agendao_config::Config>,
    ...
    match tokens[0].to_ascii_lowercase().as_str() {
        "add" => {
            ...
            let mut config = agendao_config::ModelConfig::default();
...
pub(super) async fn cli_capture_voice_prompt(
    ...
    let config = workspace_context
        ...
        .unwrap_or_default();
    let multimodal = agendao_multimodal::MultimodalAuthority::from_config(&config);
    ...
    let capture = tokio::task::spawn_blocking(move || {
        agendao_voice::capture_voice(agendao_voice::VoiceCaptureOptions {
    ...
    let parts = agendao_multimodal::SessionPartAdapter::to_session_parts(&multimodal_parts);
    let summary =
