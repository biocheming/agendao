...
fn session_stats_usage_summary(session: &agendao_types::Session) -> SessionStatsUsageSummary {
    if let Some(snapshot) = session
        .metadata
        .get(SESSION_TELEMETRY_METADATA_KEY)
        .cloned()
        .and_then(|value| {
            serde_json::from_value::<agendao_types::SessionTelemetrySnapshot>(value).ok()
        })
    {
...
mod tests {
    ...
    fn stats_usage_prefers_persisted_snapshot_over_legacy_usage() {
        ...
        session.metadata.insert(
            agendao_session::SESSION_TELEMETRY_METADATA_KEY.to_string(),
