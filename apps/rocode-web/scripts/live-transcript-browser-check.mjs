...
const BASE_URL = process.env.ROCODE_BASE_URL ?? "http://127.0.0.1:3000";
const CHROME_BIN = process.env.CHROME_BIN ?? "google-chrome";
const CHROME_PORT = Number.parseInt(process.env.ROCODE_CHROME_PORT ?? "9224", 10);
const TIMEOUT_MS = Number.parseInt(process.env.ROCODE_SMOKE_TIMEOUT_MS ?? "30000", 10);
const WORKSPACE_DIR =
  process.env.ROCODE_WORKSPACE_DIR ?? "/home/biocheming/tests/python/rust/test/life";
const PROMPT =
  process.env.ROCODE_BROWSER_PROMPT ??
...
async function launchChrome() {
  const userDataDir = await mkdtemp(path.join(tmpdir(), "rocode-live-check-chrome-"));
  const chrome = spawn(
    CHROME_BIN,
    [
