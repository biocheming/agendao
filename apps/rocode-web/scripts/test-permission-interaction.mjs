...
const outfile = path.join(
  await fs.mkdtemp(path.join(tempDir, "permission-")),
  "permission-interaction-test.mjs",
);

try {
  await build({
    entryPoints: [path.join(rootDir, "scripts/test-permission-interaction-entry.tsx")],
    outfile,
    bundle: true,
    format: "esm",
    platform: "node",
