...
function getMermaidScriptElement(): HTMLScriptElement | null {
  return document.querySelector('script[data-agendao-mermaid-runtime="true"]');
}
...
function loadMermaidRuntime(): Promise<MermaidBrowserRuntime> {
  ...
  mermaidRuntimePromise = new Promise<MermaidBrowserRuntime>((resolve, reject) => {
    ...
    script.async = true;
    script.dataset.agendaoMermaidRuntime = "true";
...
export function createAgendaoMermaidPlugin(options: MermaidPluginOptions = {}): DiagramPlugin {
...
export const agendaoMermaidPlugin = createAgendaoMermaidPlugin();
