...
const PROVIDER_SOAK_ENABLED = process.env.AGENDAO_PROVIDER_SOAK === "1";
const PROVIDER_SOAK_TIMEOUT_MS = Number.parseInt(
  process.env.AGENDAO_PROVIDER_SOAK_TIMEOUT_MS ?? "90000",
  10,
);
const PROVIDER_SOAK_PROMPT =
  process.env.AGENDAO_PROVIDER_SOAK_PROMPT ??
...
async function launchChrome() {
  const userDataDir = await mkdtemp(path.join(tmpdir(), "agendao-phase4-chrome-"));
  const chrome = spawn(
    CHROME_BIN,
    [
...
async function maybeRunProviderSoak(client, record) {
  ...
  const modelId =
    process.env.AGENDAO_PROVIDER_SOAK_MODEL ??
