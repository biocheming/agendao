...
const CHROME_PORT = Number.parseInt(process.env.AGENDAO_CHROME_PORT ?? "9233", 10);
...
async function launchChrome() {
  const userDataDir = await mkdtemp(path.join(tmpdir(), "agendao-settings-skills-smoke-"));
  const chrome = spawn(
    CHROME_BIN,
    [
...
async function openTab(client, tabId) {
  ...
  await waitForExpression(client, `document.querySelector('[data-testid="settings-tab-${tabId}"]')?.dataset.active === "true"`);
}
...
async function assertNoFatalRender(client) {
  ...
  assert(!fatal, "skills smoke hit a fatal render error");
}
...
async function run() {
  ...
  try {
    ...
    await openTab(client, "skills");
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-panel-skills\"], [data-testid=\"settings-panel-skills-loading\"]'))");
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-skills-root\"]'))", 45000);
    await assertNoFatalRender(client);

    step = "skills overview";
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-skills-overview\"]'))");
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-skills-new-name\"]'))");
    await fillInput(client, "[data-testid='settings-skills-new-name']", "phase24-skill-smoke");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-skills-new-name"]')?.value === "phase24-skill-smoke"`);

    step = "skills hub";
    await click(client, "[data-testid='settings-skills-subtab-hub']");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-skills-subtab-hub"]')?.dataset.active === "true"`);
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-skills-hub\"]'))");
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-skills-hub-search\"]'))");
    await fillInput(client, "[data-testid='settings-skills-hub-search']", "plan");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-skills-hub-search"]')?.value === "plan"`);
    await waitForExpression(
      client,
      "Boolean(document.querySelector('[data-testid=\"settings-skills-hub-result\"], [data-testid=\"settings-skills-hub-results-empty\"], [data-testid=\"settings-skills-hub-sources-empty\"]'))",
      45000,
    );
    await waitForExpression(
      client,
      "Boolean(document.querySelector('[data-testid=\"settings-skills-managed-item\"], [data-testid=\"settings-skills-managed-empty\"]'))",
      45000,
    );

    step = "skills catalog";
    await click(client, "[data-testid='settings-skills-subtab-catalog']");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-skills-subtab-catalog"]')?.dataset.active === "true"`);
    await waitForExpression(
      client,
      "Boolean(document.querySelector('[data-testid=\"settings-skills-catalog-item\"], [data-testid=\"settings-skills-catalog-empty\"]'))",
      45000,
    );

    step = "skills governance";
    await click(client, "[data-testid='settings-skills-subtab-governance']");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-skills-subtab-governance"]')?.dataset.active === "true"`);
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-skills-governance\"]'))");
    await waitForExpression(
      client,
      "document.querySelector('[data-testid=\"settings-skills-governance\"]')?.textContent?.includes('Usage Ledger') ?? false",
      45000,
    );
    await assertNoFatalRender(client);

    console.log("settings skills smoke passed");
  } catch (error) {
    const suffix = client
      ? (() => {
...
run().catch((error) => {
  console.error(`Settings skills smoke failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
