...
async function run() {
  ...
  try {
    ...
    console.log("settings general/providers smoke passed");
  } catch (error) {
    const suffix =
      client
        ? (() => {
...
run().catch((error) => {
  console.error(`Settings general/providers smoke failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
