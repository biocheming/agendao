...
const CHROME_PORT = Number.parseInt(process.env.AGENDAO_CHROME_PORT ?? "9232", 10);
...
async function launchChrome() {
  const userDataDir = await mkdtemp(path.join(tmpdir(), "agendao-settings-memory-smoke-"));
  const chrome = spawn(
    CHROME_BIN,
    [
...
async function assertNoFatalRender(client) {
  ...
  assert(!fatal, "memory smoke hit a fatal render error");
}
...
async function run() {
  ...
  try {
    ...
    await openTab(client, "memory");
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-panel-memory\"], [data-testid=\"settings-panel-memory-loading\"]'))");
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-memory-root\"]'))", 45000);
    await waitForExpression(client, "Boolean(document.querySelector('[data-testid=\"settings-memory-search\"]'))");
    await assertNoFatalRender(client);

    step = "memory search input";
    await fillInput(client, "[data-testid='settings-memory-search']", "phase24 memory smoke");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-memory-search"]')?.value === "phase24 memory smoke"`);

    step = "memory records";
    await click(client, "[data-testid='settings-memory-subtab-records']");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-memory-subtab-records"]')?.dataset.active === "true"`);
    await waitForExpression(
      client,
      "Boolean(document.querySelector('[data-testid=\"settings-memory-record\"], [data-testid=\"settings-memory-records-empty\"]'))",
      45000,
    );

    step = "memory governance";
    await click(client, "[data-testid='settings-memory-subtab-governance']");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-memory-subtab-governance"]')?.dataset.active === "true"`);
    await waitForExpression(
      client,
      "Boolean(document.querySelector('[data-testid=\"settings-memory-rule-packs\"], [data-testid=\"settings-memory-rule-packs-empty\"]'))",
      45000,
    );
    await waitForExpression(
      client,
      "Boolean(document.querySelector('[data-testid=\"settings-memory-rule-hits\"], [data-testid=\"settings-memory-rule-hits-empty\"]'))",
      45000,
    );

    step = "memory retrieval";
    await click(client, "[data-testid='settings-memory-subtab-retrieval']");
    await waitForExpression(client, `document.querySelector('[data-testid="settings-memory-subtab-retrieval"]')?.dataset.active === "true"`);
    await waitForExpression(
      client,
      "Boolean(document.querySelector('[data-testid=\"settings-memory-retrieval-items\"], [data-testid=\"settings-memory-retrieval-empty\"]'))",
      45000,
    );
    await assertNoFatalRender(client);

    console.log("settings memory smoke passed");
  } catch (error) {
    const suffix = client
      ? (() => {
...
run().catch((error) => {
  console.error(`Settings memory smoke failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
