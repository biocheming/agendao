...
AgenDao 的 Skill 系统提供能力的完整生命周期管理：从本地创建、远程分发、artifact 缓存到托管生命周期状态机。系统以 `SkillGovernanceAuthority` 为唯一治理入口，遵循 AgenDao 宪法中的"唯一权限裁决"原则。
...
`SkillAuthority` 是本地 skill 文件的唯一读写权威。它直接管理 `.agendao/skills/` 目录下的 skill 文件。
...
AgenDao 当前的 skill 系统不只负责“安装和读取”，还负责把运行中的经验回流为更好的 skill 与 methodology。
...
这一轮计划之后，AgenDao 的 skill 治理不再停留在“装上就算存在”。现在正式收口成五条线：
...
Artifact policy 通过 `agendao.jsonc` 的 `skills.hub` 字段配置：
...
所有 CLI 命令通过 `agendao skill hub` 前缀调用。

### 查询命令

| 命令 | 说明 |
|------|------|
| `agendao skill hub status` | 查看当前托管 skill 状态 |
| `agendao skill hub managed` | 查看 managed provenance 记录 |
| `agendao skill hub usage` | 查看 canonical usage / write ledger |
| `agendao skill hub negative-entropy` | 查看 negative entropy 诊断 |
| `agendao skill hub semantic-conflicts` | 查看 semantic overlap 诊断 |
| `agendao skill hub index` | 查看 source index 缓存 |
| `agendao skill hub distributions` | 列出所有分发记录 |
| `agendao skill hub artifact-cache` | 查看 artifact 缓存状态 |
| `agendao skill hub policy` | 查看当前 artifact 策略 |
| `agendao skill hub lifecycle` | 查看所有 skill 的生命周期状态 |

### 安装/更新命令

| 命令 | 说明 |
|------|------|
| `agendao skill hub install-plan --source-id <id> --source-kind registry --locator <locator> --skill-name <name>` | 生成安装计划（不执行） |
| `agendao skill hub install-apply --session-id <session> --source-id <id> --source-kind registry --locator <locator> --skill-name <name>` | 执行安装 |
| `agendao skill hub update-apply --session-id <session> --source-id <id> --source-kind registry --locator <locator> --skill-name <name>` | 执行更新 |

### 治理动作命令

| 命令 | 说明 |
|------|------|
| `agendao skill hub review-candidates-sync --session-id <session>` | 把当前 negative-entropy review candidate 同步到 workspace-local vitality |
| `agendao skill hub semantic-conflict-review-sync --session-id <session>` | 把当前 semantic conflict review candidate 同步到 workspace-local vitality |
| `agendao skill hub vitality-set --session-id <session> --skill-name <name> --state <state> --summary <text>` | 显式设置某个 skill 的 vitality judgement |

### 卸载命令

| 命令 | 说明 |
|------|------|
| `agendao skill hub detach --session-id <session> --source-id <id> --source-kind registry --locator <locator> --skill-name <name>` | 解除托管（保留文件） |
| `agendao skill hub remove --session-id <session> --source-id <id> --source-kind registry --locator <locator> --skill-name <name>` | 完全删除 |
...
- CLI: `agendao skill proposal list --status draft`
- CLI: `agendao skill proposal show <id>`
- CLI: `agendao skill proposal approve <id>`
- CLI: `agendao skill proposal reject <id>`
