...
- 首选直接使用 canonical 字段表达目标结构。
- `body` / `methodology` / `content` 是正文层字段。
- `input` / `payload` / `arguments` 一类 wrapper 只应视为兼容性恢复入口，而不是推荐写法。
- 错误提示会返回 machine-consumable repair hint；不要反过来把这些恢复别名写进主流程 prompt。
