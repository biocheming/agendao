...
Model Context Protocol 是一个基于 JSON-RPC 2.0 的协议，用于将 AgenDao 连接到外部工具、资源和提示词服务器。MCP 服务器扩展了智能体的能力范围 -- 无需修改 AgenDao 本身即可暴露文件系统、数据库、API、浏览器自动化等能力。
...
AgenDao 在握手阶段从已连接的 MCP 服务器发现工具、资源和提示词，将它们包装为原生 `PluginTool` 实例，使其对查询循环透明可用。
...
默认传输方式。AgenDao 将服务器作为子进程启动，通过其 stdin/stdout 使用换行分隔的 JSON-RPC 2.0 通信。
...
MCP 服务器在 `agendao.jsonc` 配置文件的 `mcp` 字段中声明。这是一个从服务器名称到 `McpServerConfig` 的映射。

### 项目级配置

在项目根目录创建 `agendao.jsonc`：
...
# 指定超时
agendao mcp add slow-server --url https://slow.example.com/sse --timeout 30000
```
...
对于需要 OAuth 认证的远程 MCP 服务器，AgenDao 提供内建的 OAuth 流程支持。
...
```bash
agendao mcp auth my-server --code <authorization-code>
```
...
当 MCP 服务器断开或连接失败时，AgenDao 自动启动后台重连循环：
...
使用 `agendao mcp connect <name>` 可以取消正在进行的重连循环并立即启动新的连接尝试。
