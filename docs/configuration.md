...
ROCode 通过分层的 JSON/JSONC 配置系统进行配置。本文档描述 `rocode.jsonc` / `rocode.json` 中的配置选项。
...
| 全局配置 | `~/.config/rocode/rocode.jsonc` / `~/.config/rocode/rocode.json` | 中 |
| 项目 `.rocode` 目录 | `<project>/.rocode/rocode.jsonc` / `<project>/.rocode/rocode.json` | 高 |
| 项目根目录 | `<project>/rocode.jsonc` / `<project>/rocode.json` | 最高 |
