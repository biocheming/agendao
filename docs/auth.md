...
如果你需要把凭证和 provider 定义一起管理，可以在 `agendao.jsonc` 里显式写入：
...
## `agendao auth` 命令现在到底做什么

`agendao auth` 目前只是一个很窄的 CLI 辅助命令：

- `agendao auth list`
- `agendao auth login <provider> --token ...`
- `agendao auth logout <provider>`

它**只会设置或清除当前进程内的环境变量**，不会持久化到磁盘。

```bash
agendao auth list
agendao auth login openai --token sk-...
agendao auth logout openai
```
...
另外，`agendao auth login https://...` 这种 well-known URL 登录目前还没有在 Rust CLI 里接通。

### `agendao auth` 当前支持的 provider 子集
...
- 这张表描述的是 `agendao auth` 命令，不是“AgenDao 全部 provider 能力表”。
- 像 `bedrock`、`vertex` 这类 provider 往往还需要额外上下文配置；`agendao auth` 不会替你补全这些外围条件。
...
它和 `agendao auth login` 不是一条线。今天的 CLI `auth login/logout` 不会去写这个文件。

---

## 插件认证桥

AgenDao 的 TypeScript 插件可以声明 `auth`，由 server 侧把它桥接成真实认证流程。
...
如果某个 provider 依赖插件认证，这条链路通常发生在 server / web 侧，而不是靠 `agendao auth login` 完成。
...
当前 well-known 入口读取的不是 `agendao` 目录，而是历史兼容位置：

- `~/.local/share/opencode/auth.json`

也就是说，今天这块仍然带着旧路径兼容色彩。文档不再把它包装成“AgenDao 的标准持久认证入口”。
