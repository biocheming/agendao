...
ROCode 将缓存优化收敛到稳定提示面，而不是按厂商分叉 prompt 结构。closeai-compatible 协议族侧重稳定 prefix 与可选 prompt cache affinity；ethnopic / messages 协议族侧重统一 cache breakpoint 规划。scheduler continuity、artifact summary、memory anchors 与当前动态输入会被分区放置，避免本轮 tail 正文或临时 tool output 破坏可复用前缀。
