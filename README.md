...
- 版本：`v2026.4.30`
- 可执行命令：`rocode`
- 当前公开 scheduler presets：`sisyphus` / `prometheus` / `atlas` / `hephaestus` / `verifier`
...
- 通过 scheduler continuity packet 管理同会话历史、Source Anchors、Memory Anchors 与按需 hydration，避免 follow-up 任务丢失上一轮上下文
- 通过稳定 prompt prefix、输出投影、cache fingerprint 和三端 cache diagnostic 优化上下文缓存命中
...
`release-date.sh` 会把 workspace 版本更新为日期版本并调用 `sync_version.sh`；`sync_version.sh` 会同步 `Cargo.lock`、Web `package.json` / `package-lock.json` 以及受管示例 lockfile。公开文档中的版本展示需要随本次发布一起更新。

## 上下文缓存优化

ROCode 当前的缓存优化不是“多贴缓存字段”，而是围绕稳定提示面组织请求：

- closeai-compatible 协议族优先保持稳定 prefix，并在能力明确时使用 prompt cache affinity key。
- ethnopic / messages 协议族通过统一 planner 写入 cache breakpoint，避免多个层各自修改 prompt surface。
- scheduler continuity、memory anchors、artifact summary 和当前动态输入分区放置，减少稳定前缀被 tail 正文或临时 tool output 破坏。
- CLI、TUI、Web 会显示 `Cache R/W`、`Cache H/M` 以及 cache bust diagnostic，帮助定位 tools、system、message prefix 或 cache params 的变化。

详见 [上下文缓存优化](docs/context-caching.md)。
