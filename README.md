...
ROCode 现在已经是一套完整的本地编码智能体运行时，而不是一个只有聊天入口的壳。
...
这轮计划真正给 ROCode 带来的，不只是“多了几条命令”，而是几条以前容易漂移的治理线开始收口：

- provider 不再靠 npm 名、历史别名或请求选项临时猜测；`ProviderProfile`、descriptor、validation 和 runtime profile 现在是同一条 authority 语义
- session 不再把 live context、child workflow 花费和累计消耗混成一笔账；CLI、TUI、Web 现在都能读到 request/live/workflow 三本账和 context closure contract
- external adapter 不能再自己猜 session id；必须先走 owner-local provisioning，再绑定 replay / verify / run
- fork 和 subsession 的边界变得明确：child 只拿显式 packet，parent 只吸收 result / summary；显式 full-history fork 则冻结策略并把 imported history 设为只读
- skill hub 不再只有安装/删除，还多了 usage ledger、negative entropy、semantic conflict、composition relationship、runtime gate 和 proposal review
- config、provider、scheduler、skill tree 的“当前到底生效了什么”现在有只读解释面，前端不需要再各自侧取大响应或重推配置
- Web / TUI / Server 的最终消息同步链路已收口：流式 `output_block` 和最终持久化 message 会在 authoritative `session.updated` 上重新对齐，不再让某些前端长时间停留在半成品 assistant message
- skill hub search 已进入正式产品面：搜索结果会返回 source、entry、trust、maintenance、staleness 与 refresh 建议，方便 agent 侧直接走 search → install 流程，而不只是“你自己去找一个 git URL”

当前公开的 scheduler presets：
...
ROCode 当前的产品分发入口固定为单一可执行文件 `rocode`。
...
```bash
./scripts/release-date.sh 2026-05-12
