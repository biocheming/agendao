...
你会先读仓库，会怀疑自己的判断，会临时分叉，会回滚，会改计划，会隔一天再回来继续。到了那时，真正决定体验好坏的，往往是这个系统还记不记得你们到底在做什么。

编码智能体 AgenDao 关心的是当软件工作变成一段持续数小时、数天、甚至数周的对话时，系统有没有能力不丢失边界，不误解上下文，不把历史和临时噪音混成一锅。
...
AgenDao 想守住的，是边界清楚、状态有主、上下文可收束。

软件开发是一种需要连续性、责任归属和节制感的运行时。如果一套系统不能在长回合里持续澄清、压缩、回收和完成，那么它再聪明，也只会越来越乱。

所以 AgenDao 关注的是几条更慢、也更难被替代的事：
...
AgenDao 是一套完整的本地编码智能体运行时，CLI、TUI 和 Web 共享同一套 session、scheduler、tool、provider、skill、memory、telemetry authority。换句话说，它不是把"问答能力"套在代码库上，而是试着把一段真正的软件劳动组织起来。
