...
先把三个前端/服务端可执行文件一起构建出来，CLI launcher 运行时会按同目录查找 `rocode-server` 和 `rocode-tui`：

```bash
cargo build -p rocode-cli -p rocode-server -p rocode-tui
```

如果要使用 Web 前端，还需要单独构建前端应用：
...
这会把下面三个二进制并排安装到同一个 `bin` 目录：

- `~/.local/bin/rocode`
- `~/.local/bin/rocode-server`
- `~/.local/bin/rocode-tui`

如果 `apps/rocode-web/dist` 已存在，安装脚本还会把它复制到 `~/.local/share/rocode/web`，供 `rocode-server` 在运行时加载。

### 启动方式

下面这些 `cargo run -p rocode-cli -- ...` 示例默认你已经先执行过上面的三二进制构建，保证 `target/debug` 里同时存在 `rocode`、`rocode-server` 和 `rocode-tui`。
...
打包 side-by-side release bundle：
